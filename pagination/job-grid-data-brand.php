...
if(is_array($_REQUEST['info'])){
	...
	$status="status='50' and sub_status in (".$statusstr.")";
}else{
	$status="status='50'";
}
$columns = array( 
...
if($row["sub_status"]=="53"){
	$viewd="<div align='center'><a href='brand-rep-approval.php?refid=".base64_encode($row['job_no'])."&daterange=".$_REQUEST['daterange']."&product_name=".$_REQUEST['product_name']."&brand=".$_REQUEST['brand']."&modelid=".$_REQUEST['modelid']."".$pagenav."' title='view'><i class='fa fa-eye fa-lg faicon' title='view Replcament details'></i></a></div>";
}else{
	$viewd="";
}
