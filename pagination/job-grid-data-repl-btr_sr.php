...
$sql = "SELECT *";
$sql.=" FROM jobsheet_data where repl_req_flag = 'Replacement Request' and sr_repl_by_flag in ('Partner','Self') and state_id in ($access_state) and ".$locationid." and ".$status." and ".$daterange." and ".$modelid;
//echo $sql;
...
if($row["sub_status"]=="84"){
	$viewd="<div align='center'><a href='rep-approval-btr_sr.php?refid=".base64_encode($row['job_no'])."&daterange=".$_REQUEST['daterange']."&product_name=".$_REQUEST['product_name']."&brand=".$_REQUEST['brand']."&modelid=".$_REQUEST['modelid']."".$pagenav."' title='view'><i class='fa fa-eye fa-lg faicon' title='view Replcament details'></i></a></div>";
}else{
	$viewd="";
}
