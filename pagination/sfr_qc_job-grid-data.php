...
$sql = "SELECT *";
$sql.=" FROM jobsheet_data where current_location='".$_SESSION['asc_code']."' and  sub_status='416' and ".$daterange." and ".$productid." and ".$brandid." and ".$modelid;
//$sql.=" FROM price_master where ".$loc_state." and ".$product." and ".$loc_type."";
$query=mysqli_query($link1, $sql) or die("sfr_qc_job-grid-data.php: get sfr qc job Repair details");
...
$query=mysqli_query($link1, $sql) or die("sfr_qc_job-grid-data.php: get sfr qc job details");
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
	$nestedData=array(); 
		...
		$repair_icon = "<div align='center'><a href='sfr_qc_update.php?job_no=".$row['job_no']."&daterange=".$_REQUEST['daterange']."&product_name=".$_REQUEST['product_name']."&brand=".$_REQUEST['brand']."&modelid=".$_REQUEST['modelid']."".$pagenav."' title='go to repair'><i class='fa fa-wrench fa-lg faicon' title='go to repair'></i></a></div>";
