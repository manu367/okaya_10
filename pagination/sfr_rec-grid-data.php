...
$query=mysqli_query($link1, $sql) or die("sfr_rec-grid-data.php: get sfr_rec details");
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
	...
	if($row["status"]!=4){
	$nestedData[] = "<div align='center'><a href='sfr_receive.php?challan_no=".$row['challan_no']."&daterange=".$_REQUEST['daterange']."&product_name=".$_REQUEST['product_name']."&brand=".$_REQUEST['brand']."&modelid=".$_REQUEST['modelid']."".$pagenav."' title='go to repair'><i class='fa fa-wrench fa-lg faicon' title='go to repair'></i></a></div>";
	}else{
		$nestedData[] = "";
	}
