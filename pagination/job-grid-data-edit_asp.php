...
$sql = "SELECT *";
$sql.=" FROM jobsheet_data where  current_location='".$_SESSION['asc_code']."'";
...
while( $row=mysqli_fetch_array($query) ) {  
	$nestedData=array();	
	if($row["status"]!=10 && $row["status"]!=8 && $row["status"]!=6 && $row["status"]!=49 && $row["status"]!=48 && $row["status"]!="11"  &&  $row["status"]!="12"    ){
	$job_edit="<div align='center'><a href='complaint_edit_asp.php?refid=".base64_encode($row['job_no'])."".$pagenav."' title='view'><i class='fa fa-eye fa-lg faicon' title='view Job details'></i></a></div>";
}else{
	$job_edit="";
}
	$nestedData[] = $j; 
