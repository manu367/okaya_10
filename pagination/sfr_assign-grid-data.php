...
// getting total number records without any search
 $sql = "SELECT *";
 $sql.=" FROM jobsheet_data where current_location='".$_SESSION['asc_code']."' and sub_status='423' and ".$locationid ;
//$sql.=" FROM price_master where ".$loc_state." and ".$product." and ".$loc_type."";
$query=mysqli_query($link1, $sql) or die("sfr_assign-grid-data.php: get sfr assign details");
...
$query=mysqli_query($link1, $sql) or die("sfr_assign-data.php: get sfr_assign details");
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
	...
	$nestedData[] = "<div align='center'><a href='sfr_assign_update.php?job_no=".$row['job_no']."&daterange=".$_REQUEST['daterange']."&product_name=".$_REQUEST['product_name']."&brand=".$_REQUEST['brand']."&modelid=".$_REQUEST['modelid']."".$pagenav."' title='go to Assign'><i class='fa fa-wrench fa-lg faicon' title='go to Assign'></i></a></div>";
