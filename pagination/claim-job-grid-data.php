...
$sql = "SELECT *";
$sql.=" FROM job_claim_appr  where app_status = '' and ".$daterange." and ".$locationid."";
...
while( $row=mysqli_fetch_array($query) ) { 
 ...
 // preparing an array
	...
	$nestedData[] = $row['rep_lvl'];

	$nestedData[] = "<div align='center'>
	  <input id='remark'  type='text'    name='remark$row[id]' ></div>";
	  $nestedData[] = "<div align='center'> <input type='radio'  name='appr$row[id]' id='appr$row[id]' value='Y' class='required' required />Approved<input type='radio'  name='appr$row[id]' id='appr$row[id]' value='N' class='required ' required />Reject</div>";
	 	$nestedData[] = "<div align='center'><a href='job_view_claim.php?refid=".base64_encode($row['job_no'])."".$pagenav."' title='view'><i class='fa fa-eye fa-lg faicon' title='view job details'></i></a></div>";
	$data[] = $nestedData;
	$j++;
}
