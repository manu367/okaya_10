...
## selected brand
if($_REQUEST['product']!=""){
	$prd = " product_id in ('".$_REQUEST['product']."') ";
}else{
	$prd = " product_id in ($access_product) ";
}

$prd_maped_qr = mysqli_query($link1, "select mapped_brand from product_master where $prd ");
...
$sql = "SELECT *";
$sql.=" FROM partcode_master where brand_id in (".$access_brand.") and brand_id in ('".$b."') and status='1' and ".$mod_id;
...
$sql = "SELECT *";
$sql.=" FROM partcode_master where brand_id in (".$access_brand.") and brand_id in ('".$b."')  and  status='1' ";
