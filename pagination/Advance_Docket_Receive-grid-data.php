...
$columns = array( 
	...
	while( $row=mysqli_fetch_array($query) ) {  // preparing an array
		...
		if($row["status"]=="Pending"){ $icn = "<i class='fa fa-shopping-bag lg' ></i>";}else{$icn = "<i class='fa fa-eye lg' ></i>";}
		
		$nestedData[] = "<div align='center'><a href='Receive_Advance_Docket.php?refid=".base64_encode($row['id'])."&daterange=".$_REQUEST['daterange']."&status=".$_REQUEST['status']."&assign_to=".$_REQUEST['assign_to']."".$pagenav."' title='Receive Docket'>".$icn." </a></div>";
