...
// getting total number records without any search
 ...
 $sql.=" FROM stn_master where po_type='RETURN-FROM-ENG' and  from_location = '".$_SESSION['asc_code']."' or to_location='".$_SESSION['asc_code']."'  and ".$daterange."  ";
...
if($row["status"] == "2") { $status = "Processed" ;} elseif ($row["status"] == "4") { $status = "Received" ;}
 // preparing an array
	...
	$nestedData[] = $status;
		$nestedData[] = "<div align='center'><a href='challan_issue_part_back.php?refid=".base64_encode($row['challan_no'])."&daterange=".$_REQUEST['daterange']."&status=".$_REQUEST['status']."&doc_type=".$_REQUEST['doc_type']."".$pagenav."' title='view'><i class='fa fa-eye fa-lg faicon' title='view document details'></i></a></div>";
