...
$access_location = getAccessLocation($_SESSION['userid'],$link1);
//print_r($access_state);exit;
...
if(is_array($_REQUEST['info'])){
	...
	$status="status in('70') and sub_status in ('70') and job_no in (select old_job_no from dop_serial_change_request where approve_status in ('','70','72')) ";
	//$status = "1";
}else{
	$status="status in('70') and sub_status in ('70') and job_no in (select old_job_no from dop_serial_change_request where approve_status in ('','70','72'))";
	//$status = "1";
...
$sql = "SELECT *";
//$sql.=" FROM jobsheet_data where ".$locationid." and state_id in ($access_state) and current_location in($access_location) and ".$status." and ".$daterange." and ".$modelid;
$sql.=" FROM jobsheet_data where ".$locationid." and state_id in ($access_state) and current_location in($access_location) and ".$status." and ".$daterange." and ".$modelid;
//echo $sql;
...
if($row["sub_status"]=="70"){
	if($row["status"] != "10" && $row["status"] != "12"){
		$viewd="<div align='center'><a href='rep-approval_req_serial_change.php?refid=".base64_encode($row['job_no'])."&daterange=".$_REQUEST['daterange']."&product_name=".$_REQUEST['product_name']."&brand=".$_REQUEST['brand']."&modelid=".$_REQUEST['modelid']."".$pagenav."' title='view'><i class='fa fa-eye fa-lg faicon' title='view Replcament details'></i></a></div>";
	}else{
		$viewd="";
	}
