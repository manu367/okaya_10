...
$sql = "SELECT *";
$sql.=" FROM jobsheet_data where repl_req_flag = 'Replacement Request' and repl_prd = 'Inverter' and state_id in ($access_state) and ".$locationid." and ".$status." and ".$daterange." and ".$modelid." and ".$apr_rej_st;
//echo $sql;exit;   //and (ext7='' or ext7 <= '90')
...
if($row["sub_status"]=="81" || $row["sub_status"]=="10"){
	$viewd="<div align='center'><a href='rep-approval-inverter.php?refid=".base64_encode($row['job_no'])."&daterange=".$_REQUEST['daterange']."&product_name=".$_REQUEST['product_name']."&brand=".$_REQUEST['brand']."&modelid=".$_REQUEST['modelid']."".$pagenav."' title='view'><i class='fa fa-eye fa-lg faicon' title='view Replcament details'></i></a></div>";
}else{
	$viewd="";
}
