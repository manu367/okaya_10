...
## selected  status 
if($_REQUEST['status']!=""){
	$status = " missing > 0  and missing_reco='".$_REQUEST['status']."'";
}else{
	$status = " missing > 0  and missing_reco='R'";
}
...
while($row=mysqli_fetch_array($query) ) { 
	if($row['missing_reco']=="R"){
		...
		$div="<div align='center'><a href='asp_reco_miss_req_update_view.php?refid=".base64_encode($row['challan_no'])."&f_location=".$row['to_location']."".$pagenav."' title='print'><i class='fa fa-eye fa-lg faicon' title='view  details'></i></a></div>";
