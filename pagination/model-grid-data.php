...
$sql = "SELECT *";
$sql.=" FROM model_master where ".$status."".$brandfiltter."";
...
if( !empty($requestData['search']['value']) ) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter
	...
	$sql.=" OR modelcode LIKE '".$requestData['search']['value']."%')";
}
$query=mysqli_query($link1, $sql) or die("model-grid-data.php: get model master");
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
	...
	$nestedData[] = "<div align='center'><a href='add_model.php?op=Edit&refid=".base64_encode($row['model_id'])."&status=".$_REQUEST['status']."".$pagenav."' title='view'><i class='fa fa-eye fa-lg faicon' title='view/edit model details'></i></a></div>";
