...
$sql = "SELECT *";
$sql.=" FROM billing_master where ".$condition."and (po_type = 'Sale Return'  or po_type = 'P2C') and status='4' ";
$query=mysqli_query($link1, $sql) or die("cn-parts-grid-data.php: get  details");
...
if( !empty($requestData['search']['value']) ) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter
	...
	$sql.=" OR request_no LIKE '".$requestData['search']['value']."%' )";
}
$query=mysqli_query($link1, $sql) or die("cn-parts-grid-data.php: get  details");
...
if($row["status"] == "2") {$st = "Processed" ;}
 ...
 // preparing an array
	...
	if($row["status"]==4){
				...
				$nestedData[] = "<div align='center'><a href='cn_genrate.php?refid=".base64_encode($row['challan_no'])."".$pagenav."' title='print'><i class='fa fa-eye fa-lg faicon' title='view  details'></i></a></div>";
