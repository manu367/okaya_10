...
if($_REQUEST['status']=="missing"){
	$status = " missing > 0 and (type!='PNA' and type!='PO')";
}else if($_REQUEST['status']=="broken"){
	$status = " broken > 0 and (type!='PNA' and type!='PO')";
}else{
}
...
if($_REQUEST['status']=="missing"){
	$mss="<div align='center'><a href='reconcilation_view_wh.php?refid=".base64_encode($row['challan_no'])."".$pagenav."' title='print'><i class='fa fa-eye fa-lg faicon' title='view  details'></i></a></div>";
	if($row['missing_reco']==""){
		$st="Pending For Reconcilation Request";
		$div="<div align='center'><a href='reco_miss_req_view_wh.php?refid=".base64_encode($row['challan_no'])."&f_location=".$row['from_location']."".$pagenav."' title='print'><i class='fa fa-eye fa-lg faicon' title='view  details'></i></a></div>";
	...
	if($row['damage_reco']==""){
		...
		$div="<div align='center'><a href='reco_dmd_req_view_wh.php?refid=".base64_encode($row['challan_no'])."&f_location=".$row['from_location']."".$pagenav."' title='print'><i class='fa fa-eye fa-lg faicon' title='view  details'></i></a></div>";
