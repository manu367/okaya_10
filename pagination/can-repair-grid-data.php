...
$sql.=" FROM client_inventory a, partcode_master b where a.location_code='".$_SESSION['asc_code']."'  and (b.part_category!='SPARE' and b.part_category!='ACCESSORY' ) and  a.partcode=b.partcode and ".$productid." and ".$brandid." and ".$modelid." group by a.partcode";
//$sql.=" FROM price_master where ".$loc_state." and ".$product." and ".$loc_type."";
$query=mysqli_query($link1, $sql) or die("can-repair-grid-data.php: get stock details1");
...
$query=mysqli_query($link1, $sql) or die("can-repair-grid-data.php: get stock details");
$totalFiltered = mysqli_num_rows($query); // when there is a search parameter then we have to modify total number filtered rows as per search result. 
$sql.=" ORDER BY ". $columns[$requestData['order'][0]['column']]."   ".$requestData['order'][0]['dir']."  LIMIT ".$requestData['start']." ,".$requestData['length']."   ";
/* $requestData['order'][0]['column'] contains colmun index, $requestData['order'][0]['dir'] contains order such as asc/desc  */	
$query=mysqli_query($link1, $sql) or die("can-repair-grid-data.php: get stock details2");
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
	...
	$nestedData[] = "<div align='center'><a href='imei_can_selection.php?partcode=".base64_encode($row['partcode'])."".$pagenav."' title='go to repair'><i class='fa fa-wrench fa-lg faicon' title='go to repair'></i></a></div>";
	$nestedData[] = "<div align='center'><a href='imei_repair_selection.php?partcode=".base64_encode($row['partcode'])."".$pagenav."' title='go to repair'><i class='fa fa-wrench fa-lg faicon' title='go to repair'></i></a></div>";
