...
if(is_array($_REQUEST['info'])){
	...
	$status=" status in ('1','2','7','5','3','5','56')";
}
$columns = array( 
...
$sql = "SELECT *";
$sql.=" FROM jobsheet_data where current_location='".$_SESSION['asc_code']."' and doa_rej_rmk !='' and ".$status." and ".$daterange." and ".$productid." and ".$brandid." and ".$modelid;
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
	...
	if($row['vistor_date'] !='0000-00-00'  ){
	$repair_icon = "<div align='center'><a href='asc_vistior_details.php?refid=".base64_encode($row['job_no'])."&daterange=".$_REQUEST['daterange']."&product_name=".$_REQUEST['product_name']."&brand=".$_REQUEST['brand']."&modelid=".$_REQUEST['modelid']."".$pagenav."' title='go to handover'><i class='fa fa-handshake-o fa-lg faicon' title='go to handover'></i></a></div>";
}else{
		$repair_icon="";
		}
	$nestedData[] = $j; 
