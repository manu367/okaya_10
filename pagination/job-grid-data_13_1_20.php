...
$sql = "SELECT *";
$sql.=" FROM jobsheet_data where current_location='".$_SESSION['asc_code']."' and ".$status." and ".$daterange." and ".$productid." and ".$brandid." and ".$modelid;
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
	...
	if($row['eng_id']=='' && $row["doa_rej_rmk"]!='' ){
		...
		$engid =getAnyDetails($row["eng_id"],"locusername","userloginid","locationuser_master",$link1);
			...
			else if(($row["status"]==1 || $row["status"]==2 || $row["status"]==3 || $row["status"]==7 || $row["status"]==5 || $row["sub_status"]=="52" || $row["status"]=="54"||  $row["sub_status"]=="51" ||  $row["status"]=="55"  )&& $row["call_for"]=="Workshop"  ){
	if($row['eng_id']==''){
