...
$sql = "SELECT *";
$sql.=" FROM jobsheet_data where repl_req_flag = 'Replacement Request' and repl_prd = '' and aging_lock_in_days > '90' and state_id in ($access_state) and ".$locationid." and ".$status." and ".$daterange." and ".$modelid;
...
if($row["sub_status"]=="81"){
	$viewd="<div align='center'><a href='rep-approval-btr-next.php?refid=".base64_encode($row['job_no'])."&daterange=".$_REQUEST['daterange']."&product_name=".$_REQUEST['product_name']."&brand=".$_REQUEST['brand']."&modelid=".$_REQUEST['modelid']."".$pagenav."' title='view'><i class='fa fa-eye fa-lg faicon' title='view Replcament details'></i></a></div>";
}else{
	$viewd="";
}
