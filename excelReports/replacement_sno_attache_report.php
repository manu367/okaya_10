...
$apr_pending_status = base64_decode($_REQUEST['apr_pending_status']);
////// filters value/////
...
///// status
## selected  Status
//print_r($apr_pending_status);exit;
if(is_array($_REQUEST['info']!='')){
	...
	$status="status in('84') and sub_status in ('84')";
}else if($apr_pending_status=='approved'){
	$status="status in('10') and l3_status='10' and repl_platform='HO'";
}else{
	$status="status in('84')";
}
//print_r('ddddddddddd');
...
$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue('AS1', 'Approval Date')
			->setCellValue('AT1', 'Request For')
			//->setCellValue('AU1', 'Approval Remark');
	      
            ->setCellValue('AU1', 'L1/L2 Approval Remark')
	        ->setCellValue('AV1', 'L1/L2 Rejected By')
	        ->setCellValue('AW1', 'L1/L2 Rejected Remarks')
	        ->setCellValue('AX1', 'Goodwill Type')
            ->setCellValue('AY1', 'Goodwill Approved by')
	        ->setCellValue('AZ1', 'Goodwill Updated by')
	        ->setCellValue('BA1', 'Goodwill Remarks')
	        ->setCellValue('BB1', 'Sr.No Replaced by')
            ->setCellValue('BC1', 'Replace Serial No')
            ->setCellValue('BD1', 'Replace Model')
            ->setCellValue('BE1', 'Serial No updated by')
            ->setCellValue('BF1', 'Approved by(Name)');
...
//$sql_loc = mysqli_query($link1,"Select * from jobsheet_data where ext1 = 'Replacement Request' and (ext7='' or ext7 <= '90') and ".$locationcode." and ".$status." and ".$daterange." and ".$productid." and ".$brandid." and ".$modelid." and ".$apr_rej_st);
//echo "Select * from jobsheet_data where ext1 = 'Replacement Request' and ext13 = 'Partner'  and ".$locationcode." and ".$status." and ".$daterange." and ".$modelid;exit;
$sql_loc = mysqli_query($link1,"Select * from jobsheet_data where ext1 = 'Replacement Request' and ext13 = 'Partner' and state_id in ($access_state) and ".$locationcode." and ".$status." and ".$daterange." and ".$modelid);
...
if(mysqli_num_rows($jhr)>0){
	...
	/*
	if($solution_given==''){
		$solution_given1 =getAnyDetails($row_loc['problem_detect'], "rep_desc", "rep_code", "repaircode_master", $link1); 
		}else{
	   $solution_given1 = $solution_given;
	}
	if($problem_observed==''){
		$problem_observed1 = getAnyDetails($row_loc['ext6'], "defect_desc", "defect_code", "defect_master", $link1); 
		}else{
	   $problem_observed1 = $problem_observed;
	}*/
		
	//$rejected_by = getAnyDetails(83, "updated_by", "status", "call_history", $link1);
	$rejected_by = mysqli_fetch_array(mysqli_query($link1, "SELECT updated_by,remark FROM call_history WHERE job_no = '".$row_loc['job_no']."' AND status='83' order by id desc "));
	$sr_update_by = mysqli_fetch_array(mysqli_query($link1, "SELECT updated_by,remark FROM call_history WHERE job_no = '".$row_loc['job_no']."' AND status='84' and (outcome='Request Approved - Without Serial' or outcome='Request Approved - Serial Replaced') ORDER BY id DESC "));
	//$rejected_rmk = getAnyDetails(83, "remark", "status", "call_history", $link1);
	//$approved_by = getAnyDetails(83, "updated_by", "status", "call_history", $link1);
	//$approved_rmk = getAnyDetails('83', "remark", "status", "call_history", $link1);
	//$goodwill_updated_by = getAnyDetails(84, "updated_by", "status", "call_history", $link1);
	//$goodwill_rmk = getAnyDetails(84, "remark", "status", "call_history", $link1);
	//echo "SELECT updated_by,remark FROM call_history WHERE job_no = '".$row_loc['job_no']."' AND status='82' order by id desc limit 1 ";exit;
	/*$goodwill_updated_by = mysqli_fetch_array(mysqli_query($link1, "SELECT updated_by,remark FROM call_history WHERE job_no = '".$row_loc['job_no']."' AND status='82' order by id desc limit 1 "));
	if($goodwill_updated_by['updated_by']!=''){
	$goodwill_updated_by = $goodwill_updated_by['updated_by'];
	$goodwill_rmk = $goodwill_updated_by['remark'];
	}else{
	$goodwill_updated_by = "";
	$goodwill_rmk = "";	
	}*/
	//echo "SELECT doa_remark FROM jobsheet_data WHERE job_no = '".$row_loc['job_no']."' AND ext8='Goodwill'";exit;
	//SELECT doa_remark FROM `jobsheet_data` WHERE `job_no` LIKE 'MJCUP20241225200869' and ext8 like 'Goodwill' ORDER BY job_id DESC;
	$goodwill_rmk = mysqli_fetch_array(mysqli_query($link1, "SELECT doa_remark,ext13 FROM `jobsheet_data` WHERE `job_no` LIKE  '".$row_loc['job_no']."' and ext8 like 'Goodwill' ORDER BY job_id DESC "));
	$replace_sr_and_model = mysqli_fetch_array(mysqli_query($link1, "SELECT replace_serial_no,replace_model_id FROM `replacement_data` WHERE `job_no` LIKE  '".$row_loc['job_no']."' and repl_platform='HO' ORDER BY job_id DESC "));
	...
	$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue('AS'.$i,  $row_loc['doa_ar_dt'])
			->setCellValue('AT'.$i,  $resst)
			//->setCellValue('AU'.$i,  $row_loc['app_reason']);
	        ->setCellValue('AU'.$i,  $row_loc['doa_remark'])
	     
	        ->setCellValue('AV'.$i,  $rejected_by['updated_by'])
		    ->setCellValue('AW'.$i,  $rejected_by['remark'])
		    ->setCellValue('AX'.$i,  $row_loc['goodwill_type'])
		    ->setCellValue('AY'.$i,  $row_loc['goodwill_by'])
		    ->setCellValue('AZ'.$i,  $row_loc['goodwill_appr_by'])
			->setCellValue('BA'.$i,  $goodwill_rmk['doa_remark'])
			->setCellValue('BB'.$i,  $goodwill_rmk['ext13'])
	        ->setCellValue('BC'.$i,  $replace_sr_and_model['replace_serial_no'])
	        ->setCellValue('BD'.$i,  $replace_sr_and_model['replace_model_id'])
	        ->setCellValue('BE'.$i,  $sr_update_by['updated_by'])
		    ->setCellValue('BF'.$i,  getAnyDetails($sr_update_by['updated_by'],"name","username","admin_users",$link1));
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('replacement_sno_att');
...
header('Content-Disposition: attachment;filename="replacement_sno_attache_report.xlsx"');
