...
$job_row=mysqli_fetch_array(mysqli_query($conn,"select job_no,open_date,imei from jobsheet_data where job_no='".$_REQUEST['job_no']."'"));
 $serial_no=$job_row['imei'];
    $rowk=mysqli_query($conn,"select close_date, job_no  from jobsheet_data where imei = '".$serial_no."' and status in ('10','6') and imei not like 'Z%' and close_date!='0000-00-00' and close_date > ('".$job_row['open_date']."' - INTERVAL 30 day) order by job_id ASC"); 
...
$sql_data=mysqli_query($conn,"select fault_code,repair_code,partcode from repair_detail where job_no='".$row["job_no"]."' AND partcode !='39'");
$myObj = array();
