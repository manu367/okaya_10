	...
	if($sql_chk['status']=='2' || $sql_chk['status']=='3'){
		...
		for($i=0;$i<$len;$i++){
			...
			if($eng_id!='' &&  $challan_no!='' && $sqlitem_chk['pty_receive_date']=='0000-00-00' && $sqlitem_chk['status']!='4'){
				...
				$res_stn=mysqli_query($conn,"UPDATE stn_items SET status='4', rec_qty='".$totalqty."', pty_receive_date='".$today."', opration_rmk='".$remarks."' WHERE to_location='".$eng_id."' AND challan_no='".$challan_no."' AND partcode='".$partcode."'") or die(mysqli_error($conn));
        		...
        		$res_faulty = mysqli_query($conn,"SELECT id FROM user_inventory WHERE locationuser_code='".$eng_id."' AND partcode='".$partcode."'");
	 			...
	 			if($chk_faulty > 0){
		   		...
		   		if (!$res_query) {
        			...
        			$res_query2=mysqli_query($conn,"INSERT INTO user_inventory SET location_code='".$sql_chk['from_location']."', okqty='".$totalqty."',locationuser_code='".$eng_id."',partcode='".$partcode."',part_name='".cleanData($part_name)."', brand_id='".$rs_part["brand_id"]."', product_id='".$rs_part["product_id"]."'");
		...
		for($j=0;$j<$len2;$j++){
			...
			$res_engserial=mysqli_query($conn,"insert into serial_details_eng set serial_no='".$serial_no1."',partcode='".$part_serial."',location_code='',locationuser_code='".$eng_id."',status='1',stock_type='Fresh',entry_date='".$today."',receive_date='".$today."',rec_challan_no='".$challan_no."',rec_type='".$type."',rec_img_name='".$part_img."' ");
