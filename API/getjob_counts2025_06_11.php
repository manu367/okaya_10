...
$pickDrop_jobs=$db->getJobSumpickup();
...
if($row2["inst_done"]!=''){$closed_install=$row2["inst_done"];}else{$closed_install='0';}   
 ...
 $row3=mysqli_fetch_array($pickDrop_jobs);
if($row3["pickup"]!=''){$pickup=$row3["pickup"];}else{$pickup='0';} 
if($row3["drp"]!=''){$drop=$row3["drp"];}else{$drop='0';}   
 
$pending_jobs=($open_count+$pending_count+$pna_count+$ep_count+$wip_count);
$b["pending_jobs"]="".$pending_jobs."";

$b["closed_jobs"]="".$closed_btrcount."";

$b["pfa_jobs"]="".$pfa_btrcount."";
