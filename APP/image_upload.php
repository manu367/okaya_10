 ...
 $job_row=explode("~",$job_details);
	if($data[$i]->job_no==$job_row[0] &&  $ei==$job_row[1]){
		...
		$base3=$data[$i]->encodedString3;
			$base4=$data[$i]->encodedStringSerial;
				$base5=$data[$i]->encodedString5;
		...
		$filename3 = $data[$i]->fileName3;
			$fileNameSerial= $data[$i]->fileNameSerial;
				$filename5 = $data[$i]->fileName5;
		...
		$file_nm1='../handset_image/'.$filename1;
		$file_nm2='../handset_image/'.$filename2;
		$file_nm3='../handset_image/'.$filename3;
		$file_nm4='../handset_image/'.$fileNameSerial ;
		$file_nm4='../handset_image/'.$filename5;
		...
		$image_up = $db->storeImageJob($data[$i]->job_no,$data[$i]->fileName1,$data[$i]->fileName2,$data[$i]->fileName3,$data[$i]->fileNameSerial,$data[$i]->fileName5);
		if($image_up){
		$b["id"] = $data[$i]->job_no;         
$b["status"] = 1;
array_push($a,$b);
