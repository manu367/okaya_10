...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	$jobno = $job_dt."".str_pad($nextjobno,4,0,STR_PAD_LEFT);
...
$statearea = getAnyDetails($locationstate,"zoneid","stateid","state_master",$link1); 
...
$jobresult=mysqli_query($link1,"update  jobsheet_data set job_count='".$maxcount."' where address='".ucwords($address)."' and open_date='".$today."' ");
  ...
  $smk= base64_encode($msg);
   if($phone1!=''){
      $sms_msg="Dear ".ucwords($customer_name).",your complaint has been registered and the reference no is . ".$jobno." and shall be closed within 48 hours.If you are satisfied with our service, please share this Happy code ".$hpcode." with service executive. Our service executive ".$row_loc['locationname']." Mobile No ".$row_loc['contactno1']." shall attend your complaint shortly ";
}
    
	mysqli_close($link1);

 header("location:complaint_save_back.php?msg=".$smk."&chkflag=".$cflag."&chkmsg=".$cmsg."".$pagenav."&smsmsg=".base64_encode($sms_msg)."&mobileno=".$phone1."&customer_id=".$customer_id."&imei_serial=".$imei_serial1."&email_id=".$email."&status=1");
	//exit;
...
$job_sql_t=mysqli_query($link1,"SELECT current_location FROM jobsheet_data where customer_id= '".$row_customer['customer_id']."'");
 ...
 <option value="">--Please Select--</option>
			<?php	while($br_cust = mysqli_fetch_array($check_cust)){
				...
				?>
                      ...
                      <div class="col-md-6">
  <input name="pincode" type="text" class="digits form-control"  <?php if($_SESSION['id_type']=='CC'){?>  onKeyup="getmaploc(this.value);getmapinstate(this.value)"<?php }?>maxlength="6" id="pincode" value="<?=$row_customer['pincode']?>" >
  ...
  <span class="red_small">OR You can use state/city/area option to find pincode</span>
                  ...
                  <div class="form-group">
                   ...
                   <div class="form-group">
                    ...
                    <div class="col-md-6"><label class="col-md-6 custom_label">Area<span class="red_small">*</span></label>
                     ...
                     <select name="locationarea" id="locationarea" class="form-control required" required >
                  ...
                  <div class="form-group">
                      ...
                      <div class="col-md-6" id="loc_pincode">
                            ...
                            <?php   // echo   $lctype_query="select location_code,locationname  from location_master where  location_code = '".$job_det['current_location']."' order by locationname";?>
							  
							   <select name="rep_location" id="rep_location" class="form-control required " required>

                        <?php
                       $lctype_query="select location_code,locationname  from location_master where  location_code = '".$job_det_t['current_location']."' order by locationname";
