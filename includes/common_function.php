...
function getAnyDetails($keyid,$fld,$lookupname,$tbname,$link1){
    ...
    if($keyid!=''){
		...
		//$arr1=array($fld);
            ...
            $sql="select ".$fields." from ".$tbname." where ".$lookupname." = '".$keyid."'";
            $res_tb = mysqli_query($link1,$sql) or die("ER1:".mysqli_error($link1));
...
$sql_str=mysqli_query($link1,$sql);
...
function getEngInventory($eng_id,$partcode,$fields,$link1){

	$explodee=explode(",",$fields);

   	$user_details = mysqli_fetch_array(mysqli_query($link1,"select ".$fields." from user_inventory where locationuser_code='".$eng_id."' and partcode='".$partcode."'"));
...
function sendSMSByURL1($mobile_no,$msg,$template_id,$dlt_chain){
//print_r($mobile_no);exit;
							  ...
							  CURLOPT_POSTFIELDS => json_encode(array('sender' => 'OKAYAG','receiver' => $mobile_no,'content' =>$msg,'ref_id' => '','pe_id' => '1701159118771676055','template_id' => $template_id,'msg_type' => 'TEXT','dlt_chain'=> $dlt_chain)),
...
function sendSMSByURL2($mobile_no,$msg,$template_id,$dlt_chain){
	/*
//print_r($mobile_no);exit;
							  $curl = curl_init();
							  curl_setopt_array($curl, array(
...
*/
		...
		$response=file_get_contents("https://beta.okaya.cancrm.in/includes/sendSMS_common_function.php?mobile_no=".$mobile_no."&msg=".urlencode($msg)."&template_id=".$template_id."&dlt_chain=".$dlt_chain."");
echo $response;
...
function getDispatch_type($val){
   if($val=='SAME_LOCATION'){
	 $return_val='Dispatched from Same Requested Location';  
   }elseif($val=='OTHER_LOCATION'){
	    $return_val='Dispatched from Other Location';
   }
	elseif($val=='OTHER_ENGINEER_SAME_LOCATION'){
		$return_val='Dispatched from Other Engineer on Same Requested Location';
   }
    return $return_val;
}

function storeTracationLed($link1,$job_no,$activity,$status,$update_by,$assigned,$remark,$reason){
$today_1=date("Y-m-d H:i:s");
mysqli_query($link1,"insert into led_query_history set job_no='$job_no', activity='$activity', status='$status', updated_by='$update_by', assigned_to='$assigned', remark='$remark', reason='$reason', update_date='$today_1'")or die(mysqli_error($link1));
}

function LedCustomerRecord($link1,$id,$field){
	$leddetail=mysqli_query($link1,"select $field from led_customer_master where id='$id'");
	$leddata=mysqli_fetch_array($leddetail);
	return $leddata[$field];
	
}

function getStatus_led($statusid){
	if($statusid=="0"){ $mystatus="Open";}
else if($statusid=="2"){ $mystatus="Assigned";}
else if($statusid=="1"){ $mystatus="Funnel/Pending";}
else if($statusid=="3"){ $mystatus="Convert";}
else if($statusid=="4"){ $mystatus="RWR";}
else if($statusid=="5"){ $mystatus="BER";}
else if($statusid=="6"){ $mystatus="Closed";}
else if($statusid=="7"){ $mystatus="Handover";}
else if($statusid=="11"){ $mystatus="Non Contactable";}
else {$mystatus="NA";}
return $mystatus;
}	

function storeTracationDLTT($link1,$job_no,$activity,$status,$update_by,$assigned,$remark,$reason){
$today_1=date("Y-m-d H:i:s");
mysqli_query($link1,"insert into dltt_history set created_id='$job_no', activity='$activity', status='$status', updated_by='$update_by', assigned_to='$assigned', remark='$remark', reason='$reason', update_date='$today_1'")or die(mysqli_error($link1));
}

function getLocationId($link1,$locationid,$field){
$str="";
$rs=mysqli_query($link1,"select $field from location_master where locationid='$locationid'")or die(mysqli_error($link1));
...
function getQueryType($statusid){
if($statusid=="0"){ $mystatus="Query";}
else if($statusid=="1"){ $mystatus="Complaint";}
else if($statusid=="2"){ $mystatus="Information Request";}
else if($statusid=="3"){ $mystatus="Others";}
else {$mystatus="NA";}
return $mystatus;
}

function getUserId($link1,$userid,$field){
$str="";
$rs=mysqli_query($link1,"select $field from admin_users where username='$userid'")or die(mysqli_error($link1));
while($row=mysqli_fetch_array($rs)){
$str.=$row[$field];
}
return $str;
...
function getModelId($link1,$modelid,$field){
$str="";
$rs=mysqli_query($link1,"select $field from model_master where modelcode='$modelid'")or die(mysqli_error($link1));
$row=mysqli_fetch_array($rs);
$str.=$row[$field];
return $str;
}

function getPartDetail($link1,$part,$field){
$str="";
$rs=mysqli_query($link1,"select $field from partcode_master where partcode='$part'")or die(mysqli_error($link1));
$row=mysqli_fetch_array($rs);
$str.=$row[0];
return $str;
}

function getStateId($link1,$stateid,$field){
$str="";
$rs=mysqli_query($link1,"select $field from state_master where stateid='$stateid'")or die(mysqli_error($link1));
while($row=mysqli_fetch_array($rs)){
$str.=$row[$field];
}
return $str;
...
function getCity_led($link1,$cityid,$field){
$str="";
$rs=mysqli_query($link1,"select $field from city_master where  cityid = '$cityid' ")or die(mysqli_error($link1));
while($row=mysqli_fetch_array($rs)){
$str.=$row[$field];
}
return $str;
...
function getStatus($statusid){
	if($statusid=="0"){ $mystatus="Open";}
else if($statusid=="1"){ $mystatus="Pending";}
else if($statusid=="2"){ $mystatus="Pending";}
else if($statusid=="3"){ $mystatus="Repaired";}
else if($statusid=="4"){ $mystatus="QC Done";}
else if($statusid=="5"){ $mystatus="BER";}
else if($statusid=="6"){ $mystatus="Closed";}
else if($statusid=="7"){ $mystatus="Handover";}
else if($statusid=="8"){ $mystatus="Approval Pending";}
else if($statusid=="9"){ $mystatus="Approved";}
else if($statusid=="10"){ $mystatus="Rejected";}
else if($statusid=="11"){ $mystatus="Non Contactable";}
else {$mystatus="NA";}
return $mystatus;
}

function getLedReasonId($link1,$reasonid,$field){
$str="";
$rs=mysqli_query($link1,"select $field from led_reason_master where reasonid='$reasonid'")or die(mysqli_error($link1));
while($row=mysqli_fetch_array($rs)){
$str.=$row[$field];
}
return $str;
...
/////////////////////////////////////////////////
function getLedCloseReasonId($link1,$reasonid,$field){
$str="";
$rs=mysqli_query($link1,"select $field from led_close_reason_master where reasonid='$reasonid'")or die(mysqli_error($link1));
while($row=mysqli_fetch_array($rs)){
$str.=$row[$field];
}
return $str;
...
function getCityID($link1,$cityid){
$str="";
$rs=mysqli_query($link1,"select city from city_master where cityid='$cityid'")or die(mysqli_error($link1));
while($row=mysqli_fetch_array($rs)){
$str.=$row[city];
}
return $str;
...
function getUserCode($link1,$userid,$field){
$str="";
$rs=mysqli_query($link1,"select $field from admin_users where username='$userid' and status='1'")or die(mysqli_error($link1));
while($row=mysqli_fetch_array($rs)){
$str.=$row[$field];
}
return $str;
...
function getDivision($link1,$did,$field){
$rs=mysqli_fetch_array(mysqli_query($link1,"select $field from division_master where id='$did'"));
return $rs[$field];
}

##### User Name as per UserID
function getUser_Name($userid,$link1){

	$res_user=mysqli_query($link1,"select id,locusername from locationuser_master where userloginid='".$userid."'")or die(mysqli_error());
	
	$res_admin=mysqli_query($link1,"select id,name from admin_users where username='".$userid."'")or die(mysqli_error());

	if(mysqli_num_rows($res_user)>0){

       $row_user=mysqli_fetch_assoc($res_user);
	   $username=$row_user['locusername'];

	}
	else if(mysqli_num_rows($res_admin)>0){

       $row_admin=mysqli_fetch_assoc($res_admin);
	   $username=$row_admin['name'];

	}
	else{

	$res_location=mysqli_query($link1,"select locationid,locationname from location_master where location_code='".$userid."'")or die(mysqli_error());
	   $row_location=mysqli_fetch_assoc($res_location);
	   $username=$row_location['locationname'];
...
////// function to get Faulty in-transit by Eng

function getENGFaulty_Intransit($eng_id, $partcode, $link1){
