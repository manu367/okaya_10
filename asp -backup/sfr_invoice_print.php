...
$po_sql = "SELECT * FROM sfr_challan where challan_no='" . $docid . "'";
...
<!--	<page size="A4" layout="portrait"></page>-->
	<page size="A4">
        ...
        <div align="center" class="lable"><u><strong><?php echo "Delivery Challan"; $str = "Challan";?></strong></u></div>
      <table class="table" border="1" style="margin-bottom: 0px;">
            <tbody>
              <tr>
                ...
                <td width="35%" colspan="2"><?=dt_format($po_row['challan_date'])?></td>
              </tr>
              <tr>
			  ...
			  <td colspan="2"><strong>Address</strong></td>
                <td colspan="2"><?=$po_row['from_address'];?></td>
                <td colspan="2"><strong>Address</strong></td>
                <td colspan="2"><?=$po_row['to_address'];?></td>				
              </tr>
              <tr>
             ...
             <tr>
                <td colspan="2"><strong>Courier Name</strong></td>
		...
		<table class="table" border="1" style="margin-bottom: 0px;">
          <thead>
              ...
              <td width="10%"><strong>IMEI</strong></td>
              ...
              <?php if($po_row['to_state']==$po_row['from_state']){ ?>
          ...
          <tbody>
			...
			$podata_sql="SELECT * FROM sfr_transaction where challan_no='".$docid."'";
			$podata_res=mysqli_query($link1,$podata_sql);
			while($podata_row=mysqli_fetch_assoc($podata_res)){
				$val = $podata_row['qty']*$podata_row['price'];
                $taxable = $val-$podata_row['discount_amt']*$podata_row['qty'];
				$product_name = explode("~", getAnyDetails($podata_row['part_id'],"part_name,product_id,brand_id,hsn_code,vendor_partcode","partcode","partcode_master",$link1)); 
			?>
              <tr>
                <td><?=$i?></td>
                <td><?=$product_name[0].' | '.$podata_row['part_id'].' | '.$product_name[4];?></td>
                <td><?=$product_name[3];?></td>
                <td><?=$podata_row['imei']?></td>
                ...
                <?php }?>
				<td><?=currencyFormat($podata_row['total_cost'])?></td>       
                </tr>
            <?php
			$total+=$podata_row['qty'];
			$price+=$podata_row['price'];
			$value+=$podata_row['basic_amt'];                                                
			$discount = $podata_row['discount'];
			$i++;
			}
			if($po_row['to_state']==$po_row['from_state']){ $colspn=12; }else{ $colspn=10;}
			?>   
            	<tr>
		...
		<table class="table" border="1">
           <tbody>         
              ...
              <tr>        
                <td colspan="8" style="border-top:none" align="right">(signature)</td>
              </tr>
              <tr>
