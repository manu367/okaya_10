...
if($result_find_count > 0){
	...
	if ($custo_id==""){	
		...
		$usr_add="insert into customer_master set  customer_id='".$customer_id."', customer_name='".ucwords($customer_name)."', address1='".ucwords($address)."', pincode='".$pincode."', cityid='".$locationcity."', stateid='".$locationstate."', email='".$email."', mobile='".$phone1."', alt_mobile='".$phone2."', update_date='".$today."', update_by='".$_SESSION['asc_code']."',max_id='".$code_id."',type='".$customer_type."',custarea='".$locationarea."'";
		...
		$usr_add="update customer_master set customer_name='".ucwords($customer_name)."', address1='".ucwords($address)."', pincode='".$pincode."', cityid='".$locationcity."', stateid='".$locationstate."', email='".$email."', alt_mobile='".$phone2."', update_date='".$today."', update_by='".$_SESSION['asc_code']."',type='".$customer_type."',custarea='".$locationarea."'  where   customer_id='".$custo_id."'";
		$res_add=mysqli_query($link1,$usr_add); 
		$cust_id=$custo_id;
	}
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//$sql_inst = "INSERT INTO jobsheet_data set job_no='".$jobno."', system_date='".$today."', location_code='".$_SESSION['asc_code']."', city_id='".$locationcity."', state_id='".$locationstate."', pincode='".$pincode."', product_id='".$product_name."', brand_id='".$brand."', customer_type='".$customer_type."', model_id='".$modelsplit[0]."', model='".$modelsplit[1]."', imei='".$imei_serial1."', open_date='".$today."', open_time='".$currtime."', warranty_status='".$st_wart."',warranty_days='".$wsd."', dop='".$pop_date."', dname='".ucwords($dealer_name)."', inv_no='".$invoice_no."',  call_type='".$call_type."', call_for='".$call_for."', customer_name='".ucwords($customer_name)."',  contact_no='".$phone1."', alternate_no='".$phone2."', email='".$email."', address='".ucwords($address)."', cust_problem='".$voc1."', cust_problem2='".$array_voc2."', cust_problem3='".$voc3."', phy_cond='".$physical_cond."', created_by='".$_SESSION['userid']."', remark='".ucwords($remark)."', ".$st." ,ip='".$ip."',current_location='".$st_asp."',customer_id='".$cust_id."',entity_type='".$entity_type."',acc_rec='".$array_accprest."',area_type='".$arr_pin['area_type']."',pen_status='2',area='".$area."',partcode='".$row_part['partcode']."',h_code='".$hpcode."',custarea='".$locationarea."',installation_date ='".$install_date."',eng_id='".$eng_id1."'";
	$sql_inst = "INSERT INTO jobsheet_data set job_no='".$jobno."', system_date='".$today."', location_code='".$_SESSION['asc_code']."', city_id='".$locationcity."', state_id='".$locationstate."', pincode='".$pincode."', product_id='".$product_name."', brand_id='".$brand."', customer_type='".$customer_type."', model_id='".$modelsplit[0]."', model='".$modelsplit[1]."', imei='".$imei_serial1."', open_date='".$today."', open_time='".$currtime."', warranty_status='".$st_wart."', dop='".$pop_date."',call_type='".$call_type."', call_for='".$call_for."', customer_name='".ucwords($customer_name)."',  contact_no='".$phone1."', alternate_no='".$phone2."', email='".$email."', address='".ucwords($address)."', cust_problem='".$voc1."', cust_problem2='".$array_voc2."', cust_problem3='".$voc3."', phy_cond='".$physical_cond."', created_by='".$_SESSION['userid']."', remark='".ucwords($remark)."', ".$st." ,ip='".$ip."',current_location='".$st_asp."',customer_id='".$cust_id."',area_type='".$arr_pin['area_type']."',pen_status='2',area='".$area."',partcode='".$row_part['partcode']."',h_code='".$hpcode."',custarea='".$locationarea."',installation_date ='".$install_date."',eng_id='".$eng_id1."',warranty_days='".$wp_days."'";
...
//print_r('dddddddd');exit;
	...
	if ($flag) {
		...
		$sms_msg_qt = "Dear Customer, Thanks for using our products, your Service Request No: $jobno has been registered with us. Our Service Team will get in touch with you shortly, Team OKAYA";	
		
		$template_id='1707170816011730067';
		
		$sms_resp = sendSMSByURL1($phone1,$sms_msg_qt,$template_id); 
  ...
  $(document).ready(function(){
	...
	$('#brand').change(function(){
	  ...
	  $.ajax({
		...
		//alert(data);
	    $('#modeldiv').html(data);
		$('#modeldiv select.selectpicker').selectpicker('refresh');
...
function getWarrantySlab(wslabcode){
  ...
  function checkSerialdeatil(){
    ...
    const serialNo = document.getElementById('imei_serial1').value.trim();
    const goBtn = document.getElementById('search_sr');
    const originalText = goBtn ? goBtn.value : 'Go';
    ...
    // Dummy Serial Number Case
    // Dummy Serial Number Case
if (serialNo === 'ZZZZZZZZZZ') {
    console.log('Dummy Serial Detected — reloading full product list for manual selection');

    // Clear brand & model dropdowns
    $('#brand').val('');
    $('#modelid').html("<option value=''>--Select Model--</option>");

    // Reload full product list dynamically (server-rendered)
    $.ajax({
        url: window.location.pathname, // reload same PHP file
        type: 'GET',
        data: { imei_serial: 'ZZZZZZZZZZ' },
        success: function (html) {
            // Extract only the productdiv HTML
            const newProductDiv = $(html).find('#productdiv').html();
            $('#productdiv').html(newProductDiv);
            $('#productdiv select.selectpicker').selectpicker('refresh');
            alert('Please select Product, Brand, and Model manually.');
        },
        error: function () {
            alert('Unable to reload product list.');
        }
    });
    ...
    const payload = {
        serial_no: serialNo,
        accessKey: '862fb088559a6bb31cdc099deb1fa0e50d56150dcb4e72028fc133f09da11cea'
    };
    ...
    .then(data => {
        ...
        if (data.status === "1" || (data.msg && data.msg.toLowerCase() === "success")) {

            // --- BRAND ---
            const brandSelect = document.getElementById('brand');
            if (brandSelect) {
                brandSelect.value = data.brand_id;
                if ($(brandSelect).hasClass('selectpicker')) {
            ...
            if (productSelect) {
                ...
                opt.textContent = data.product;
                productSelect.appendChild(opt);
                productSelect.value = data.product_id;
                if ($(productSelect).hasClass('selectpicker')) {
            ...
            if (modelSelect) {
                ...
                const opt = document.createElement('option');
                opt.value = data.model_id;
                opt.textContent = data.model || data.model_code || 'Model Not Found';
                modelSelect.appendChild(opt);
                modelSelect.value = data.model_id;
                if ($(modelSelect).hasClass('selectpicker')) {
                    $(modelSelect).selectpicker('refresh');
                }
            }

            console.log('✅ Serial info applied successfully');
        } else {
            alert(data.message || data.msg || 'No data found for this serial number');
        }
    })
    .catch(err => {
        console.error('🔥 JS Error:', err);
        alert('Unable to fetch serial details. Please try again.');
    })
    .finally(() => {
        if (goBtn) {
            goBtn.disabled = false;
            goBtn.value = originalText;
        }
    });
...
function loadAllProductsAndModels() {
    // 1️⃣ Load all products into dropdown
    const productSelect = document.getElementById('product_name');
    const modelSelect = document.getElementById('modelid');
    ...
    // Fetch all products via AJAX from existing PHP endpoint (already available in your code)
    $.ajax({
        url: '../API/get_all_products.php', // ✅ you can create this lightweight API if not exists
        type: 'GET',
        dataType: 'json',
        success: function (data) {
            if (Array.isArray(data) && data.length > 0) {
                ...
                console.warn('No products found in list');
            }
        },
        complete: function () {
    ...
    // 2️⃣ Attach dynamic model loading when product changes
    $(productSelect).off('change').on('change', function () {
        const prodId = $(this).val();
        if (prodId) {
            getprdwisebrand(prodId); // already in your PHP file
        }
    });

    alert('Dummy Serial detected — you can now choose any Product and Model.');
}
            ...
            <div class="panel panel-info">
              ...
              <div class="panel-body">
              	  ...
              	  <div class="form-group">
              	    <div class="col-md-6">
              	      <label class="col-md-6 custom_label">Customer Type <span class="red_small">*</span></label>
                      <div class="col-md-6">
...
if($row_customer['type']==""){?>
 <option value="">--Please Select--</option>
  ...
  <span class="red_small">OR You can use state/city/area option to find pincode</span>
    ...
    <input name="imei_serial1" id="imei_serial1" type="text" value="<?=$_REQUEST['imei_serial']?>" class="form-control required alphanumeric" minlength='10' maxlength="26"  style="width:160px;text-transform: uppercase;float: left;" <?php if($product_det['serial_no']!=''){?> readonly <?php }else{}?> required />
	...
	<!--<input type="button" name="search_sr" id="search_sr" style="float: left;margin-top: 4px;border: 1px solid #225702; border-radius: 4px; background-color: #225702; color: #f8f3f6; font-weight: bold; margin-left:5px;" onClick="return getSerialdeatils();" value="Go!" />-->
				...
				<input type="button" name="search_sr" id="search_sr" style="float: left;margin-top: 4px;border: 1px solid #225702; border-radius: 4px; background-color: #225702; color: #f8f3f6; font-weight: bold; margin-left:5px;" onClick="getSerialdeatils();checkSerialdeatil();" value="Go!" />
                      </div>
...
<div class="col-md-6" id="productdiv">
        ...
        onChange="resetProdModel();getprdwisebrand(this.value);" required>

<?php
$entered_serial = isset($_REQUEST['imei_serial']) ? trim($_REQUEST['imei_serial']) : '';

if ($entered_serial === 'ZZZZZZZZZZ') {
    // 🔹 Dummy serial entered → show full product list
    echo "<option value=''>--Select Product--</option>";
    $dept_query = "SELECT * FROM product_master 
                   WHERE status = '1' 
                   AND product_id IN (".$access_product.") 
                   ORDER BY product_name";
    $check_dept = mysqli_query($link1, $dept_query);
    while ($br_dept = mysqli_fetch_array($check_dept)) {
        echo "<option value='".$br_dept['product_id']."'>".$br_dept['product_name']."</option>";
    }
}
elseif (!empty($product_det['product_id'])) {
    // 🔹 Actual serial case → show only auto-detected product
    $prod_name = getAnyDetails($product_det['product_id'], "product_name", "product_id", "product_master", $link1);
    echo "<option value='".$product_det['product_id']."'>".$prod_name."</option>";
} 
else {
    // 🔹 Default blank dropdown
    echo "<option value=''>--Select Product--</option>";
}
?>
</select>
                      ...
                      <div class="col-md-6">
                       ...
                       <select name="call_type" id="call_type" class="form-control required" required>
                            ...
                            <option value='carry in'>Carry In</option>
                              ...
                              <option value='HO Visit'>HO Visit</option>
                              <option value='missed call'>Missed Call</option>
                              <option value='Revalidation'>Revalidation</option>
                              <option value='sms'>SMS</option>
                              <option value='toll free no'>Toll Free No</option>
                              <option value='whatsapp'>Whatsapp</option>
                              <option value='vitrual no'>Virtual No</option>
                             <option value='WH Visit'>WH Visit</option>
