...
$error_msg="";
	...
	if($_REQUEST['els_status']!='OK' && $_POST['warranty_status']=='IN'){
		$ws="VOID";
		$els="ELS Fail at Higher Level";
	}else{
		$ws=$_POST['warranty_status'];
		$els="ELS Pass at Higher Level";
		}

   //////////////////////////////// Insert call  history//////////////////////////////////////
    $flag2 = callHistory($_POST['job_no'],$_SESSION['asc_code'],"423",$els,"ELS Done at L3",$_SESSION['userid'],$ws,$_REQUEST['els_status'],"","",$ip,$link1,$flag);

	  
	  /////////////////////////////////////Job Sheet data////////////////////////////////////////////////////////
	 
   $up_job=mysqli_query($link1,"update jobsheet_data set sub_status='423'  where job_no='".$_POST['job_no']."'" );
 ...
 header("location:sfr_els_job.php?msg=".$msg."&chkflag=".$cflag."&chkmsg=".$cmsg."".$pagenav);
  ...
  $cflag="success";
		$cmsg="Success";
		$msg="You have successfully ELS  against job no ".$_REQUEST['job_no'];
		mysqli_commit($link1);
	header("location:sfr_els_job.php?msg=".$msg."&chkflag=".$cflag."&chkmsg=".$cmsg."".$pagenav);
  }
       ...
       <table class="table table-bordered" width="100%">
            ...
            <tr>
              ...
              <td ><?=$job_row['remark']?></td>
			   <td><label class="control-label">ElS Status </label></td>
              <td> <select name="els_status" id="els_status" class="form-control requried">
                 <option value=""<?php if($_POST['els_sta']=='') echo " Selected"?>>Please Select ELS Status--</option>
                          <option value="OK"<?php if($_POST['els_sta']=='OK') echo " Selected"?>>OK</option>
                        
                            <option value="Physical Damaged"<?php if($_POST['els_sta']=='Physical Damaged') echo " Selected"?>>Physical Damaged</option>
                      
                      
                          <option value="Tempered"<?php if($_POST['els_sta']=='Tempered') echo " Selected"?>>Tempered</option>
                          <option value="Liquid Damaged"<?php if($_POST['els_sta']=='Liquid Damaged') echo " Selected"?>>Liquid Damaged</option>
                        
                         <option value="Burnt"<?php if($_POST['els_sta']=='Burnt') echo " Selected"?>>Burnt</option>
                         <option value="FAIL"<?php if($_POST['els_sta']=='FAIL') echo " Selected"?>>FAIL</option>
                 </select></td>
            </tr>
            <tr>
