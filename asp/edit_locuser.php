...
<head>
 ...
 <script>
    ...
    $(document).ready(function () {
        $('#dob').datepicker({
            format: "yyyy-mm-dd", endDate: "<?=$today?>", todayHighlight: true, autoclose: true});

        $('#doj').datepicker({format: "yyyy-mm-dd",
			//endDate: "<?//=$today?>",
			todayHighlight: true,
			autoclose: true
		});
...
<div class="container-fluid">
  <div class="row content">
            ...
            <div class="tab-content">
                    ...
                    <form  name="frm2" id="frm2" class="form-horizontal" action="" method="post">
                        <div class="table-responsive">
                            <table id="myTable" class="table table-hover">
                                ...
                                <tbody>
                                <?php
                                $query_12="select tabid from access_tab where userid='".$row_locdet['userloginid']."' and tabid='".$row_report['tabid']."' and status='1'";
                                ...
                                while($row_report=mysqli_fetch_array($rs_report)){
                                    ...
                                    $state_acc=mysqli_query($link1,$query_12)or die(mysqli_error());
                                    ...
                                    <td>
                                        <input
                                                ...
                                                id="report1<?=$j?>"
                                                name="report[]"
                                                value="<?=$row_report['tabid']?>" <?php if($num1 > 0) echo "checked";?> /><?=$row_report['subtabname']?>
                                    </td>
                                    <?php if($i/4==0){?>
                                        </tr>
                                        <?php
              ...
              <form  name="frm1" id="frm1" class="form-horizontal" action="" method="post">
                  <div class="form-group">
                      <div class="col-md-12">
                      <?php
                      $sql_pincode="select * from location_pincode_access where location_code='".$row_locdet['userloginid']."' and statusid='1' group by pincode";
                      $res_pin = mysqli_query($link1,$sql_pincode);
    ...
    <script>
        function UserMapper(){
            this.name='1234567789000';
            this.password='null';
            this.email='pathakmanu@gmail.com';
            this.contact=6395896677;
            this.dateofbirth='<?= date('Y-m-d')?>';
            this.date_of_joining='<?= date('Y-m-d')?>';
            this.status=false;
            this.type=null;
        }
        function validator(obj, successCallback, errorCallback) {
            ...
            if (!obj.name) {
                obj.name = "null";
            } else if (obj.name.length > 12) {
                obj.name = obj.name.slice(0, 12);
            }
            ...
            if (!obj.email) {
                ...
                errors.email_length = "Email too small";
            ...
            } else if (!contactPattern.test(obj.contact)) {
                errors.contact = "Contact must contain only numbers";
            } else if (obj.contact.length !== 10) {
                errors.contactLength = "Contact must be 10 digits";
            }

            if (!obj.dob || !obj.doj) {
                errors.date = "DOB and DOJ required";
            } else {
                let dob = new Date(obj.dob);
                let doj = new Date(obj.doj);
                if (isNaN(dob) || isNaN(doj)) {
                    errors.date = "Invalid date";
                } else {
                    let ageDiff = doj.getFullYear() - dob.getFullYear();
                    if (ageDiff < 21) {
                        errors.age = "Joining age must be at least 21 years";
                    }
                }
            }
        ...
        UserMapper.prototype.validate=function(fn,efn){
            validator(this,fn,efn);
        }

        function TabPermission(){
            UserMapper.call(this);
            this.check=false
        }
        TabPermission.prototype.checked=function(){
            this.check=true;
        }

        TabPermission.prototype = Object.create(UserMapper.prototype);
        TabPermission.prototype.constructor = TabPermission;

        function PincodeMapping(){
            TabPermission.call(this);
        }
        PincodeMapping.prototype=Object.create(TabPermission.prototype);
        PincodeMapping.prototype.constructor=PincodeMapping;
        function Mapper(){
            PincodeMapping.call(this);
        }
        Mapper.prototype=Object.create(PincodeMapping.prototype);
