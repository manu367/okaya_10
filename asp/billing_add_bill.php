    ...
    function getAvlStk(indx){
        ...
        $.ajax({
            ...
            success:function(data){
                ...
                else {document.getElementById("avl_stock["+getdata[1]+"]").value="0";}
            }
        });
        $.ajax({
            type:'post',
        ...
        <div class="container-fluid">
            ...
            <div class="row content">
                ...
                <div class="<?=$screenwidth?>">
                    ...
                    <?php }?>
                  </select></td><td><select name="brand[0]" id="brand[0]" class="form-control required" onChange="getmodel(0)" style="width:60px;text-align:left;padding: 2px" required>
...
<script>
    ...
    function RetailInvoice(value = "") {

    }
    RetailInvoice.prototype.getInvoice=function(arr=[],sum=0){
        if(Array.isArray(arr) && !isNaN(sum)){
            let left=0;
            let right=arr.length-1;
            if(right==0 || right<0)return;
            while (left<right){
                let result=arr[left]+arr[right];
                if(result===sum){return [arr[left],arr[right],result];}
    ...
    const algo = new RetailInvoice("invoice");
    console.log(algo.getInvoice([1,3,5,12,12,89,90], 90));
</script>
    </body>
