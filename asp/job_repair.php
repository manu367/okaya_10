...
$model_flags = explode("~",getAnyDetails($job_row['model_id'],"make_doa,repairable,replacement,replace_days,out_warranty,wp","model_id","model_master",$link1));
...
############################################ Symptom Code #######################################################
$rs_symp=mysqli_query($link1,"select * from symptom_master where status='1' and product_id='".$job_row['product_id']."' order by symp_desc")or die("Error-> in symptom code".mysqli_error($link1));
...
function addNewRowPna(findex){
   var ni = document.getElementById('myDivPna');
    ...
    //// check if previous row is filled with these three values (fault code,repair code, part)/////
       ...
       var newdiv = document.createElement('div');
       newdiv.setAttribute("id",divIdName);
	   newdiv.innerHTML = "<i class='fa fa-close fa-lg' onclick='return getRowDisablePna("+num+")' title='Remove this row'></i><table width='100%' class='table table-bordered'><tbody><tr><td><select name='pending_part["+num+"]' class='form-control' id='pending_part["+num+"]' style='width:250px;'  onChange='duplicate_entry("+num+",this.value)';><option value='' selected='selected'> Select Pending Part</option><?php $rs_pna = mysqli_query($link1,"select partcode, part_name, part_category from partcode_master  where  model_id like '%".$job_row['model_id']."%' and status='1'      and partcode not in  ( select partcode from client_inventory  where location_code='".$_SESSION['asc_code']."' and okqty > 0)  order by part_name")or die("error in pna dropdown".mysqli_error($link1));while($row_pna = mysqli_fetch_array($rs_pna)){?><option value='<?=$row_pna[0]?>'><?=$row_pna[1]."-".$row_pna[2]?> (<?=$row_pna[0]?> ) </option><?php }?></select><span id='errmsg["+num+"]' name='errmsg["+num+"]' class='red_small'></span></td></tr></tbody></table>";
       ni.appendChild(newdiv);
	    if(num==4){ 
   ...
   var part_det="";
    ...
    var totrows=document.getElementById('theValue').value;
	  ...
	  $.ajax({
	    type:'post',
		url:'../includes/getAzaxFields.php',
		data:{repaircode:val, modelcode:'<?=$job_row['model_id']?>', locationcode:'<?=$_SESSION['asc_code']?>', indxno:indx, part_inf:part_det},
		success:function(data){
        ...
        <div class="form-group"  id="page-wrap" style="margin-left:10px;" >
          ...
          <div class="tab-content">
              ...
              <table class="table table-bordered" width="100%">
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>ghghhj<?=$job_row['cust_problem']?></td>
                    <td><?=$job_row['cust_problem2']?></td>
            ...
            <div class="form-group">
                <div class="col-md-6"><label class="col-md-5">ELS Status <span class="red_small">*</span></label>
                  <div class="col-md-6">
                    <select name="els_status" id="els_status" class="form-control required" required onChange="getWarranty();">

                          <option value='OK' <?php if($job_row['els_status']=='OK'){ echo 'selected';}?>>OK</option>

                          <option value="Physical damaged" <?php if($job_row['els_status']=='Physical damaged'){ echo 'selected';}?>>Physical damaged</option>

                          <option value="Tempered" <?php if($job_row['els_status']=='Tempered'){ echo 'selected';}?>>Tempered</option>

                          <option value="Liquid damaged" <?php if($job_row['els_status']=='Liquid damaged'){ echo 'selected';}?>>Liquid damaged</option>

                          <option value="Electrical malfunctioning" <?php if($job_row['els_status']=='Electrical malfunctioning'){ echo 'selected';}?>>Electrical malfunctioning</option>

                    </select>
                  </div>
                </div>
                <div class="col-md-6"><label class="col-md-6">Warranty Status</label>
                  <div class="col-md-6">
                    <input name="warranty_status" id="warranty_status" type="text" value="<?php echo $job_row['warranty_status'];?>" class="form-control required" readonly/>
                  </div>
                </div>
            ...
            <div class="form-group">
                <div class="col-md-6"><label class="col-md-5">Repair Status <span class="red_small">*</span></label>
                  <div class="col-md-6">
                    <select name="jobstatus" id="jobstatus" onChange="return change_stat(this.value);" class="required form-control" style="width:250px;" required>
                      ...
                      <?php
							 $res_jobstatus = mysqli_query($link1,"select status_id, display_status from jobstatus_master where status_id=main_status_id and system_status in ('EP','PNA','Repair Done','RWR','Replacement','SFR','WIP') order by display_status");
              ...
              <?php if($job_row['call_for']=='Company Stock'){?>
               <div class="form-group">
                <div class="col-md-6"><label class="col-md-5">Stock Category<span class="red_small">*</span></label>
                  <div class="col-md-6">
                    <select name="stock_type" id="stock_type"  class="required form-control" style="width:250px;" >
                      <option value="" selected>--Select Stock Type--</option>
                      <?php
							 $res_part = mysqli_query($link1,"select part_name,partcode from partcode_master where  (part_category!='SPARE' and part_category!='ACCESSORY' ) and model_id like '%".$job_row['model_id']."%' ");
							 while($row_part = mysqli_fetch_assoc($res_part)){
							?>
                      <option value="<?=$row_part['partcode']?>" <?php if($_REQUEST['partcode'] == $row_part['partcode']){echo 'selected';}?>>
                      <?=$row_part['part_name']?>
                      </option>
                      <?php
							 }
                  		...
                  		<tr>
                    		<td>
                    			<select name="pending_part[0]" class="required form-control"  id="pending_part[0]" style="width:250px;">
									...
									$rs_pna = mysqli_query($link1,"select partcode, part_name, part_category from partcode_master  where  model_id like '%".$job_row['model_id']."%'  and status='1'     and partcode not in  ( select partcode from client_inventory  where location_code='".$_SESSION['asc_code']."' and okqty > 0)  order by part_name")or die("error in pna dropdown".mysqli_error($link1));
                      ...
                      <tbody>  
                		<tr>
                  			...
                  			<td>
                                ...
                                ?>
                              	<select  name="replace_model" class="required form-control" id="replace_model" style="width:150px;" onChange="replace_type(this.value);">
                            ...
                            <td>
                  			...
                  			<td id="dispimeifield" style="display:none;">
                            	<input name="new_imei1" id="new_imei1" type="text" class=" form-control alphanumeric"  maxlength="15" style="width:200px;" placeholder="Enter IMEI1/Serial No.1"/>
                    			<input name="new_imei2" id="new_imei2" readonly type="text" class="form-control alphanumeric" style="width:200px;" placeholder="Enter IMEI2/Serial No.2"/>
                            </td>
               	 		</tr> 
              		</table>
                  </div>
