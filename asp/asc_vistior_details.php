...
if($_POST){
	if ($_POST['hando']=='VISIT Scheduled' && $docid!=''){
		...
		if($Re_app_by==''){
		 $outcome="Visit - $vistor_date , $vistor_time ";
		 $date_res = ",res_date='".$today."'";
		 ...
		 $outcome="Visit - $vistor_date , $vistor_time ";
		  $date_res = "";
		   $activity="Visit Re-Scheduled";
		}
    echo 	 $usr_upd = "UPDATE jobsheet_data set status='56',sub_status='56', vistor_date='".$vistor_date."',vistor_time='".$vistor_time."',vistor_app_by='".$app_by."' ".$date_res."  where job_no = '".$docid."'";
...
$flag = callHistory($docid,$_SESSION['asc_code'], '56', $activity,$outcome,$_SESSION['userid'],$job_row['warranty_status'],$remark,"","",$ip,$link1,$flag);
		...
		if ($flag) {
			...
			$cflag = "success";
			$cmsg = "Success";
			$msg="You have successfully updated Vistior details for ".$docid;
		} else {
		...
		mysqli_close($link2);
	 header("location:schedule_list.php?msg=".$msg."&chkflag=".$cflag."&chkmsg=".$cmsg."".$pagenav);
    exit;
	}
   	///// move to parent page
...
<?php
if($_REQUEST['vistor_date']!='' ){?>
    $(document).ready(function () {
	  $('#vistor_date').attr('readonly', true);
	});
	<?php }else{?>
	$(document).ready(function () {
...
<div class="container-fluid">
 <div class="row content">
   ...
   <div class="<?=$screenwidth?>">
      <h2 align="center"><i class="fa fa-handshake-o"></i>Reschedule</h2>
   ...
   <div class="panel-group">
    ...
    <div class="panel panel-info table-responsive">
      <div class="panel-heading"><i class="fa fa-handshake-o fa-lg"></i>&nbsp;&nbsp;Visit Scheduled</div>
      <div class="panel-body">
		<form id="frm1" name="frm1" class="form-horizontal" action="" method="post">
          <div class="form-group">
            <div class="col-md-10"><label class="col-md-4 control-label">Visit Date <span class="red_small">*</span></label>
              <div class="col-md-6" ><div style="display:inline-block;float:left;">
                     
                     <input type="text" class="form-control required" name="vistor_date"  id="vistor_date" style="width:150px;" value="<?php if($job_row['vistor_date']!='' && $job_row['vistor_date']!='0000-00-00'  ){echo $job_row['vistor_date']; } else{}?>" >
          ...
          <div class="form-group">
            <div class="col-md-10"><label class="col-md-4 control-label"> Visit Time <span class="red_small">*</span></label>
              <div class="col-md-6">
              
                <input type="time" id="vistor_time" name="vistor_time" required class="form-control" value="<?php if($job_row['vistor_time']!='' && $job_row['vistor_time']!='00:00:00' ){echo $job_row['vistor_time']; } else{}?>"  /> 
              </div>
            </div>
          </div>
          <div class="form-group">
            <div class="col-md-10"><label class="col-md-4 control-label"> Approved By <span class="red_small">*</span></label>
              <div class="col-md-6">
                <input type="text" name="app_by" required id="app_by" class="form-control" value="<?=$job_row['vistor_app_by']?>"  />
				<input type="hidden" name="Re_app_by"  id="Re_app_by"  value="<?=$job_row['vistor_app_by']?>" />
              </div>
            </div>
          </div>
		        <div class="form-group">
          ...
          <div class="form-group">
            <div class="col-md-12" align="center">
              <input type="submit" class="btn<?=$btncolor?>" name="hando" id="hando" value="VISIT Scheduled" title="Visit Scheduled" <?php if($_POST['hando']=='Handover'){?>disabled<?php }?>>&nbsp;&nbsp;&nbsp;
              ...
              <input title="Back" type="button" class="btn<?=$btncolor?>" value="Back" onClick="window.location.href='schedule_list.php?daterange=<?=$_REQUEST['daterange']?>&product_name=<?=$_REQUEST['product_name']?>&brand=<?=$_REQUEST['brand']?>&modelid=<?=$_REQUEST['modelid']?><?=$pagenav?>'">
            </div>
          </div>
    </form>
      </div><!--close panel body-->
