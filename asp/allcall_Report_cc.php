...
<script>
    ...
    const date_rng=document.getElementById("date_rng");
    const report=new Report();

    submit_button.addEventListener("click",function (event) {
        // event.preventDefault();
        // console.log(report.urlcreate(date_rng.valuue,report.getStatus()));
    ...
    Report.prototype.urlcreate=function(date,status){
        const rname=`<?=base64_encode("allcallexcel_cc")?>`;
        const locationcode='<?=base64_encode($_REQUEST['locationname']);?>';
        const url=`excelexport.php?rname=<?=base64_encode("allcallexcel_cc")?>&pending=<?=base64_encode($_REQUEST['pending'])?>&daterange=<?=$_REQUEST['daterange']?>&location_code=<?=base64_encode($_REQUEST['locationname']);?>&modelid=<?=base64_encode($_REQUEST['model']);?>&status=<?=base64_encode($_REQUEST['status']);?>&substatus=<?=base64_encode($_REQUEST['substatus']);?>&pending=<?=base64_encode($_REQUEST['pending'])?>&state=<?=base64_encode($_REQUEST['statename']);?>&proid=<?=base64_encode($_REQUEST['prod_code']);?>&brand=<?=base64_encode($_REQUEST['brand']);?>&rdate=<?=$_REQUEST['date_type']?>`
        return url;
    }
    Report.prototype.setdate=function(){
        this.date=date_rng.value.split(" - ");
    }
    Report.prototype.getDate=function(){
        return this.date;
    }
    Report.prototype.setpending=function(){
    ...
    Report.prototype.getReport=function (){
        if(this.getStatus()){
            console.log("pending report showing");
        }else{
            console.log("All report showing");
        }
    }

    Report.prototype.SendRequest=async function(){
        await fetch("")
    }
    Report.prototype.downloadReport=function (){}
