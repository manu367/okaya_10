          ...
          <div class="col-md-6"><label class="col-md-5 control-label">Location</label>
            <div class="col-md-6">
				<select name="location_code" id="location_code" class="form-control"  >
					...
					<?php
					$res_pro = mysqli_query($link1,"select to_location from billing_master where from_location = '".$_SESSION['asc_code']."'  group by to_location"); 
					while($row_pro = mysqli_fetch_assoc($res_pro)){
					?>
					<option value="<?=$row_pro['to_location']?>" <?php if($_REQUEST['location_code'] == $row_pro['to_location']) { echo 'selected'; }?>><?=getAnyDetails($row_pro['to_location'],"locationname","location_code","location_master",$link1 )?></option>
					<?php } ?>
                </select>
            </div>
          </div>
