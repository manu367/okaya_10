	...
	function getChangedPartInfo(partcode,indx){
		...
		$.ajax({
			...
			success:function(data){
				var getValue = data.split("~");
				//alert(getValue);
				document.getElementById("asp_va"+getValue[4]).value = getValue[0];	
				document.getElementById("avl_qty"+getValue[4]).value = getValue[1];	
            ...
            <tbody>
              ...
              <tr>
                ...
                <td><select name="doc_type" id="doc_type" class="form-control">
                     ...
                     <option value="DC" selected>Delivery Challan</option>
                    </select></td>
       ...
       <table class="table table-bordered" width="100%">
            <thead>
              <tr>
                ...
                <th width="35%">Partcode</th>
                <th width="10%">Price</th>
                <th width="8%">Req./Pend. Qty</th>
                <th width="20%">Stock Type</th>
				 	 <th width="20%">ASP Avl Qty</th>
			...
			$tot_reqqty = 0;
			$tot_pendqty = 0;
			$podata_sql="SELECT * FROM po_items where po_no='".$job_row['po_no']."' and status != '5' and qty!=processed_qty";
			...
			?>
              <tr>
                ...
                <td >
                	<?php //echo $part[0]." (".$podata_row['partcode'].")"; ?>
                	
                	<select style="width: 200px;" name="chg_part<?=$podata_row['id']?>" id="chg_part<?=$i?>"  class="form-control"  onChange="getChangedPartInfo(this.value,<?=$i?>)" >
                      <option value=''>--Select Partcode--</option>
                      <?php
                        $dept_query="SELECT partcode,part_name,vendor_partcode FROM partcode_master where status = '1' and brand_id = '".$podata_row['brand_id']."' and product_id='".$podata_row['product_id']."' and model_id Like '%".$podata_row['model_id']."%'  order by partcode";
                ...
                <td align="right"><?php //currencyFormat($price_asp); ?><input class="form-control" style="width:80px;" name="price<?=$podata_row['id']?>" id="price<?=$i?>" type="text" value="<?=$price_asp?>" readonly /></td>
                <td align="right"><?=$podata_row['qty']." / ".$pend_qty?><input name="req_qty<?=$podata_row['id']?>" id="req_qty<?=$i?>" type="hidden" value="<?=$pend_qty?>"/></td>
                 <td align="center">
                 	<select name="stock_type<?=$podata_row['id']?>" id="stock_type<?=$i?>" onChange="getAblQty(<?=$i?>);"  class="form-control" style="padding: 0px 0px; width:80px;" >
                    	<option value="okqty">Purchase Stock</option>
                        <option value="consqty">Consigned Stock</option>
                    </select>
                    ...
                    <input type="hidden" name="sess_code" id="sess_code" value="<?=$_SESSION['asc_code']?>" />
                  </td>
                 <td>22<input style="width: 50px;" type="text" name="asp_va<?=$podata_row['id']?>" id="asp_va<?=$i?>" value="<?=$aspavlqty;?>" class="form-control" readonly /></td>
                <td align="right"><input name="disp_qty<?=$podata_row['id']?>" id="disp_qty<?=$i?>" type="text" class="digits form-control alert-success" required style="width:80px;text-align:right" onKeyUp="calculateline('<?=$i?>');" value='0'></td>
            ...
            <tr>
			 ...
			 <tr>
                <td align="center" colspan="13"><span id="errormsg" class="red_small"></span>
