...
$(document).ready(function() {
	var dataTable = $('#joblist-grid').DataTable( {
		...
		//"order": [[ 0, "asc" ]],
		"ajax":{
			url :"../pagination/stock-list-data-wh.php", // json datasource
        ...
        <div class="form-group">
         <div class="col-md-6"><label class="col-md-5 control-label">Location</label>	  
			<div class="col-md-6">
                ...
                <?php
                $res_pro = mysqli_query($link1,"select location_code,locationname from location_master where locationtype='WH'"); 
       ...
       <?php if ($_REQUEST['Submit']){?>
        <div class="form-group">
		  <div class="col-md-10"><label class="col-md-4 control-label"></label>	  
			<div class="col-md-6" align="left">
              <a href="../excelReports/stocklistwhexcel.php?location_code=<?=$_SESSION['asc_code']?>&product_name=<?=$_REQUEST['prod_code'];?>&brand=<?=$_REQUEST['brand']?>&modelid=<?=$_REQUEST['model']?>" title="Export Login Logout details in excel"><i class="fa fa-file-excel-o fa-2x faicon" title="Export Login Logout details in excel"></i></a>
            </div>
          </div>
	    </div><!--close form group-->
        <?php }?>
