<?php
  ...
  //////  if we want to Add new po
	 if ($_POST['add']=='Dispatch' && $_SESSION['asc_code']!=''){
      ...
      $invoice_no = $row_dccount['stn_series'].""
              .$row_dccount['fy'].""
              .str_pad($next_dcno,4,"0",STR_PAD_LEFT);
...
<script src="../js/bootstrap-select.min.js"></script>
  ...
  <?php  if ( $_REQUEST['stock_type'] == '' || $_REQUEST['locationcode'] == '' ) { ?>
       <script>$("#frm2").find("input:enabled, select:enabled, textarea:enabled").attr("disabled", "disabled");</script>
       <?php } ?>
  <?php
  ...
  <script>
      function Global(){
          ...
          this.screen=window;
          this.obj=[];
          this.createotalTable("frm1");
      }
      Global.prototype.createotalTable=function(id){
          this.obj['element']=this.doc.getElementById(id);
          this.obj['table']=this.doc.createElement("table");
