...
if($_POST['final_sub']=='PROCESS'){
	...
	while($row_serial=mysqli_fetch_array($get_serial)){
		...
		mysqli_query($link1,"INSERT INTO imei_details set imei1='".$row_serial['serial_no']."',grn_no='".$po_no."',grn_date='".$po_row['receive_date']."',partcode='".$row_serial['partcode']."',model_id='".$model_id."', location_code='".$po_row['location_code']."',status='1',entry_date='".$today."' ,stock_type ='okqty' ");
		///// update serial no. inventroy
        mysqli_query($link1,"INSERT INTO imei_history set imei1='".$row_serial['serial_no']."',partcode='".$row_serial['partcode']."',transaction_no='".$po_no."',remark='GRN Receive' ,location_code='".$_SESSION['asc_code']."' ");    
	...
	if($checkboxqty['dispqty']==$qty){
        mysqli_query($link1,"update grn_data set attach_file='BY SCANNING',imei_attach='Y' where grn_no='".$po_no."' and partcode='".$partcode."'");
	}
	$cflag = "success";
	$cmsg = "Success";
	$msg = "IMEI is successfully uploaded against.".$partcode;
	header("Location:grn_local_view.php?refid=".base64_encode($po_no)."&msg=".$msg."&chkflag=".$cflag."&chkmsg=".$cmsg."".$pagenav);
	exit;
}
?>
<!DOCTYPE html>
 ...
 <div class="row content">
   ...
   <div class="panel-group">
    ...
    <div class="panel panel-info table-responsive" style="width:50%; display:inline-block;float:right;">
         ...
         <thead>
	      ...
	      if ($str == "") {  $str.= $row['serial_no']; } else {  $str.="," . $row['serial_no'];}
            ...
            <tr>
              ...
              <td align="center"><a href='' onClick="confirmDel('deleteTemplocalGrnImei.php?refid=<?=base64_encode($po_no)?>&partcode=<?=base64_encode($partcode)?>&pqty=<?=base64_encode($qty)?>&partname=<?=$_REQUEST['partname']?>&grn_type=<?=$_REQUEST['grn_type']?>&rid=<?=$row['id']?><?=$pagenav?>', 'myWin3', 'toolbar=no, status=no, resizable=No, scrollbars=No, width=860, height=530, top=50, left=120');return false" title="Delete this imei no."><i class="fa fa-trash fa-lg faicon"></i></a></td>
            </tr>
            <?php 
  			$i++;
		  }
 	...
 	<div style="display:inline-block; float:left">
         <input title="Back" type="button" class="btn<?=$btncolor?>" value="Back" onClick="window.location.href='grn_local_view.php?refid=<?=base64_encode($po_no)?><?=$pagenav?>'">
     </div>    
