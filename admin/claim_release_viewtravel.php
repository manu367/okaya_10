...
$cr_limit = getAnyDetails($po_row["from_location"],"claim_amt","location_code","current_cr_status",$link1);
...
///// after hitting receive button ///
 if ($_POST['save']=='Release'){
	...
	{
			 $narreation= "Travel Claim Release" ."-".$_POST['claim_month'];
			 
	 /// insert  into  location_account_ledger table //////////////////////////
   $result1=mysqli_query($link1,"insert into location_account_ledger_claim set transaction_type ='".$narreation."',month_year ='".$_POST['claim_month'] ."' , crdr = 'DR' , amount = '".$_POST['relese_amt']."' , entry_date = '".$today."' , remark = '".$_POST['remark']."' , location_code = '".$_POST['location_code']."', transaction_no='".$_POST['inv_no']."' ");
	...
	/// insert  into  location_account_ledger table //////////////////////////
   $result4=mysqli_query($link1,"update billing_master set  trvel_rmk='Travel Claim Release' where  challan_no = '".$_POST['inv_no']."' ");
	//// check if query is not executed
    if (!$result4) {
	   $flag = false;
	...
	/// insert  into  location_account_ledger table //////////////////////////
   $result3=mysqli_query($link1,"update current_cr_status set  claim_amt  = claim_amt - '".$_POST['relese_amt']."'    where  location_code = '".$_POST['location_code']."' ");
	//// check if query is not executed
    if (!$result3) {
	   $flag = false;
	   $error_msg = "Error details3: " . mysqli_error($link1) . ".";
			...
			$flag=dailyActivity($docid,"","Travel Claim Release ","Claim Release",$_SERVER['REMOTE_ADDR'],$link1,$flag);
	}	
     ...
     <div class="panel-heading"><i class="fa fa-id-card fa-lg"></i>&nbsp;&nbsp;Release Amount</div>
       ...
       <table class="table table-bordered" width="100%">
            ...
            <tr>
                ...
                <input type="text" id="relese_amt" name="relese_amt" class=" form-control required" value=<?=$po_row['total_cost']?>  readonly required></td>
