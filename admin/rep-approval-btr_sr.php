...
//print_r('ddddddddddddd');exit;
			//echo "UPDATE jobsheet_data set status='10',sub_status  = '10', l3_status='10', close_date='".$today."', close_time='".$currtime."', pen_status = '6', ext14 = 'Replaced Without Serial no', repl_platform='HO'  where job_no='".$docid."' "."<br><br>";

			$jobsheet_upd = mysqli_query($link1,"UPDATE jobsheet_data set status='10',sub_status  = '10', l3_status='10', close_date='".$today."', close_time='".$currtime."', pen_status = '6', ext14 = 'Replaced Without Serial no', repl_platform='HO'  where job_no='".$docid."' ");
			...
			$flag = callHistory($docid,$job_row['current_location'],"84","Request Approved","Request Approved - Without Serial",$_SESSION['userid'],"",$remark,"","",$ip,$link1,$flag);

		}else{
			
			$image_wr_card = "";
			$image_prd = "";
//echo "select replace_serial_no, serial_no, job_no from replacement_data where replace_serial_no='".strtoupper(trim($replace_serial_no))."' and status != '12' ";exit;
			...
			if($count_serial > 0){
				...
				if($_FILES['upd_doc']["name"]!=''){
					...
					if($img_upld1 != ""){
						...
						$image_wr_card = "";
						$flag = false;
						$error_msg = "Warranty Card Image can not upload on server due to size or image type issue " . mysqli_error($link1) . ".";
					}
				}else{
					$image_wr_card = "";
				}
				...
				if($_FILES['upd_doc1']["name"]!=''){
					...
					if($img_upld1 != ""){
						...
						$image_prd = "";
						$flag = false;
						$error_msg = "Product Image can not upload on server due to size or image type issue " . mysqli_error($link1) . ".";
					}
				}else{
					$image_prd = "";
				}
				...
				if($image_wr_card != "" || $image_prd != ""){
					...
					//echo "INSERT INTO image_upload_details set job_no ='".$job_row['job_no']."', activity='Complaint Replacement Approval', img_url='".$image_wr_card."', img_url1='".$image_prd."', upload_date='".$today."',location_code='".$_SESSION['userid']."'"."<br><br>";

					$result_img = mysqli_query($link1,"INSERT INTO image_upload_details set job_no ='".$job_row['job_no']."', activity='Complaint Replacement Approval', img_url='".$image_wr_card."', img_url1='".$image_prd."', upload_date='".$today."',location_code='".$_SESSION['userid']."'");
				...
				$jobsheet_upd = mysqli_query($link1,"UPDATE jobsheet_data set status='10',sub_status  = '10', l3_status='10' ".$stttr.", close_date='".$today."', close_time='".$currtime."', pen_status = '6', repl_platform='HO'  where job_no='".$docid."' ");
				/// check if query is execute or not//
				if(!$jobsheet_upd){
					$flag = false;
					$err_msg = "Error1". mysqli_error($link1) . ".";
			...
			//	echo "INSERT INTO repair_detail SET job_id='".$old_s['job_id']."', job_no ='".$docid."', repair_location='".$old_s['current_location']."', repair_type='Replacement', location_code='".$old_s['location_code']."', model_id='".$old_s['model_id']."', eng_id ='".$old_s['eng_id']."' , status='10', remark='Battery Replaced', repair_code='', partcode='', part_qty='1', fault_code='', rep_lvl='', part_repl='',close_date='".$today."', warranty_status = '".$old_s['warranty_status']."' ,old_challan='', ref_sno='', product_id = '".$old_s['product_id']."', brand_id='".$old_s['brand_id']."', replace_serial='".strtoupper(trim($replace_serial_no))."', repl_model='".$replace_model_id."', old_serial='".$old_s['imei']."', part_cost='".$sql_model['customer_price']."',loc_on_pcb='', pcb_repairable='', app_version='".$app_version."', scan_type='M',img_name='', battery_name='',battery_other='', module = '', old_type = 'M', old_serial_fileName = '' ";exit;
				...
				$flag = callHistory($docid,$job_row['current_location'],"84","Request Approved","Request Approved - Serial Replaced",$_SESSION['userid'],"",$remark,"","",$ip,$link1,$flag);
			}
		}
		
		////// insert in activity table////
		$flag = dailyActivity($_SESSION['userid'], $docid, "84",$remark,$_SERVER['REMOTE_ADDR'], $link1, $flag);
										...
										<tbody>
											...
											<?php //if($job_row['sold_unsold']=="Sold"){ ?> 
														...
														<select id="replace_model_id" name="replace_model_id" class="form-control required" required >
															...
															<?php 
																//$dept_query="SELECT model_id,model,wp FROM model_master where status = '1' and product_id in ('11','46','50') order by model ";
																 $dept_query="SELECT model_id,model,wp FROM model_master where status = '1' and product_id in ('4','5','9','10') order by model ";
												...
												<tr>
													...
													<td><label class="control-label" >Upload Product Card</label></td>
