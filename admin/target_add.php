...
if($_POST['submit']=="Add"){
	...
	/////// generate target no //////////
	$tar_qr = mysqli_fetch_array(mysqli_query($link1, "SELECT MAX(temp_no) AS tn FROM sf_target_master "));
	$temp_id = $tar_qr[0];
	...
	$sql_master = "INSERT INTO sf_target_master SET target_no ='".$targetid."', temp_no = '".$temp_id."', month = '".$target_month."', year = '".$target_year."', target_type = '".$target_type."', period_type = 'Monthly', emp_id = '".$user_id."', entry_screen = 'FRONT', status = '".$status."', remark = '".$remark."', target_val = '".$targetval."', create_date  = '".$today."', create_by  = '".$_SESSION['userid']."'";
		...
		if((count($prod_code)>0)&&(count($target_qty)>0)){
			for($i=0; $i < count($prod_code); $i++){
				if(($prod_code[$i]!="") && ($target_qty[$i]!="")){
					...
					$sql_data = "INSERT INTO sf_target_data SET target_no = '".$targetid."', prod_code = '".$prod_code[$i]."', qty = '".$target_qty[$i]."', month = '".$target_month."', year = '".$target_year."', emp_id = '".$user_id."' ";
      ...
      <form name="frm1" id="frm1" class="form-horizontal" action="" method="post" >
		  ...
		  <div class="form-group">
              <div class="col-md-12" > 
                  ...
                  <div class="col-md-6">
				  <?php
				  	$qr = mysqli_query($link1, "SELECT username, name FROM admin_users WHERE status = 'active' order by name ");
				  ?>
                  <select name="user_id" id="user_id"  data-live-search="true" class="form-control selectpicker required" required >
					  ...
					  <option value="<?=$row['username'];?>" <?php if($_REQUEST['user_id']==$row['username']){ echo "selected"; } ?> ><?php echo $row['username']." | ".$row['name']; ?></option>
					  <?php } ?>
                  </select>
                  </div>    
              </div>  
              ...
              <div class="col-md-12" > 
                  ...
                  <div class="col-md-6">
					  <select name="target_type" id="target_type" class="form-control required" required onChange="showTargetField(this.value)" >
						  ...
						  <option value="Qty" <?php if($_REQUEST['target_type']=="Qty"){ echo "selected"; } ?> >Qty</option>
