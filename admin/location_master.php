...
<script>
    ...
    GetContent.prototype.showContent=function(selection=null){
        this.content=selection;
        console.log(this.content);
    }
    const content=new GetContent();
    document.addEventListener("selectionchange", function () {
        const selection = window.getSelection().toString().trim();
        if (selection.length > 0) {
            // console.log(selection);selection;
            content.showContent(selection);
        }
    });
    ...
    validateUser("manupatak")(function (err, validator) {
        if (err) {
            console.log(err);
        } else {
            console.log("Valid user:", validator);
        }
    });
</script>
</body>
