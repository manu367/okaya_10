...
if ($_REQUEST['op']=='Edit'){
	$sel_usr="select * from model_master where model_id='".$getid."'";
...
if($_POST){
    ...
    if ($_POST['add']=='ADD'){
    ///////// insert model data	   
    $usr_add="INSERT INTO model_master set make_job='Y', out_warranty ='N', chk_serimei ='Y',product_id ='".$product_name."', brand_id ='".$brand_name."', model='".ucwords($model_name)."' , technical_model ='".ucwords($technical_name)."',modelcode='".$model_code."',release_date='".$today."',wp='".$warranty_days."',replace_days='".$replace_days."', status='".$status."',createdate='".date("Y-m-d H:i:s")."',createby='".$_SESSION['userid']."',rep_warr_days='".$rep_warr_days."',ser_charge='".$ser_charge."'";
	...
	////// 1 entry in partcode master one for unit
	/// Unit partcode in partcode master
	///////// insert model data	   
    $usr_add="INSERT INTO partcode_master set product_id ='".$product_name."', brand_id ='".$brand_name."',model_id='".$newmodelcode."', part_name='".ucwords($model_name)."' , hsn_code ='".$newmodelcode."',part_desc='".ucwords($model_name)."',location_price='', customer_price ='', servicekit_qty='',servicekit_flag='', part_category='UNIT', part_for='ALL', repair_code='', status='".$status."',createdate='".date("Y-m-d H:i:s")."',createby='".$_SESSION['userid']."',wp='".$warranty_days."',warranty_days='".$warranty_days."'";
	$res_add=mysqli_query($link1,$usr_add);
	//// check if query is not executed
	if (!$res_add) {
		 $flag = false;
	...
	////// return message
	$msg="You have successfully created a model like ".$newmodelcode;
	$cflag="success";
	$cmsg="Success";
   }
   else if ($_POST['upd']=='Update'){ 
    $usr_upd = "UPDATE model_master set product_id ='".$product_name."', brand_id ='".$brand_name."',release_date='".$today."',wp='".$warranty_days."', replace_days='".$replace_days."', status='".$status."' , updatedate='".date("Y-m-d H:i:s")."',updateby='".$_SESSION['userid']."',ser_charge='".$ser_charge."' where model_id = '".$getid."'";
	...
	// update all details in partcode master also //
    $usr_upd = "UPDATE partcode_master set product_id ='".$product_name."', brand_id ='".$brand_name."', status='".$status."' , updatedate='".date("Y-m-d H:i:s")."',updateby='".$_SESSION['userid']."' where model_id = '".$getid."'";
    $res_upd = mysqli_query($link1,$usr_upd);
	//// check if query is not executed
	if (!$res_upd) {
		 $flag = false;
	...
	////// insert in activity table////
	$flag = dailyActivity($_SESSION['userid'],$getid,"MODEL","UPDATE",$ip,$link1,$flag);
	////// return message
	$msg="You have successfully updated model details for ".$getid;
	$cflag="success";
	$cmsg="Success";
   }else{
	    $flag = false;
...
$(document).ready(function(){
	 ...
	 function get_branddeatis() {
	  var brand_id=$('#brand_name').val();
	  $.ajax({
	    type:'post',
		url:'../includes/getAzaxFields.php',
		data:{barnddetails:brand_id},
		success:function(data){
	 ///alert(data);
		if(data!=""){
	    var brand_datils=data.split("~");
			
			//document.getElementById("len_serialno").value=brand_datils[0];
			//document.getElementById("software_version").value=brand_datils[1];
			
			//document.getElementById("rep_warr_days").value=brand_datils[3];
			
			document.getElementById("replace_days").value=brand_datils[9];
			document.getElementById("warranty_days").value=brand_datils[18];
			
			//document.getElementById("minimum_minute").value=brand_datils[20];
			///document.getElementById("cust_amount").value=brand_datils[21];
			///document.getElementById("dist_amount").value=brand_datils[22];
			///document.getElementById("dealer_amount").value=brand_datils[23];
			///document.getElementById("trems_cond").value=brand_datils[24];
			//document.getElementById("release_date").value=brand_datils[25];
			document.getElementById("ser_charge").value=brand_datils[26];
...
<div class="container-fluid">
  <div class="row content">
    ...
    <div class="<?=$screenwidth?>">
      <h2 align="center"><i class="fa fa-cube"></i> <?=$_REQUEST['op']?> Model</h2><br/><br/>
      <div class="form-group"  id="page-wrap" style="margin-left:10px;">
          ...
          <div class="form-group">
            <div class="col-md-6"><label class="col-md-6 control-label">Model Name <span class="red_small">*</span></label>
                <div class="col-md-6">
               	 <input type="text" name="model_name" class="required form-control" id="model_name" value="<?=$sel_result['model']?>" required/>
              </div>
            </div>
            <div class="col-md-6"><label class="col-md-6 control-label">Technical Name</label>
              <div class="col-md-6">
                 <input type="text" name="technical_name" class="form-control" id="technical_name" value="<?=$sel_result['technical_model']?>"/>
              </div>
            </div>
          </div>
         ...
         <?php /*?> <div class="form-group">
          ...
          <div class="form-group">
            <div class="col-md-6"><label class="col-md-6 control-label">Model Code<span class="red_small">*</span></label>
              <div class="col-md-6">
               	<input type="text" name="model_code" class="form-control" id="model_code" value="<?=$sel_result['modelcode']?>"/>
              </div>
            </div>
         <div class="col-md-6"><label class="col-md-6 control-label">Service Charges<span class="red_small">*</span></label>
          ...
          <div class="form-group">
            <div class="col-md-6"><label class="col-md-6 control-label">Customer Warranty Days <span class="red_small">*</span></label>
              <div class="col-md-6">
                 <input type="text" name="warranty_days" class="digits form-control" id="warranty_days" value="<?=$sel_result['wp']?>" required/>
              </div>
            </div>
            ...
            <div class="col-md-6"><label class="col-md-6 control-label">DOA Clased AT CWH<span class="red_small">*</span></label>
              <div class="col-md-6">
          ...
          <?php /*?><div class="form-group">
            ...
            <div class="col-md-6"><label class="col-md-6 control-label">Customer Warranty Days </label>
              <div class="col-md-6">
                 <input type="text" name="warranty_days" class="digits form-control" id="warranty_days" value="<?=$sel_result['wp']?>"/>
              </div>
            </div>
          </div><?php */?>
			  <?php /*?><div class="form-group">
            ...
            <div class="col-md-6"><label class="col-md-6 control-label">Customer Amount</label>
              <div class="col-md-6">
                 <input type="text" name="cust_amount" class="form-control number" id="cust_amount" value="<?=$sel_result['cust_amount']?>"/>
              </div>
            </div>
          </div>
			  
			  <div class="form-group">
            <div class="col-md-6"><label class="col-md-6 control-label">Distributor Amount</label>
                <div class="col-md-6">
               	  <input type="text" name="dist_amount" class="form-control number" id="dist_amount" value="<?=$sel_result['dist_amount']?>"/>
              </div>
            </div>
            <div class="col-md-6"><label class="col-md-6 control-label">Dealer Amount</label>
              <div class="col-md-6">
                 <input type="text" name="dealer_amount" class="form-control number" id="dealer_amount" value="<?=$sel_result['dealer_amount']?>"/>
              </div>
				  </div></div><?php */?>
         <?php /*?> <div class="form-group">
          ...
          <?php /*?><div class="form-group">
            <div class="col-md-6"><label class="col-md-6 control-label">Term and Conditions</label>
              <div class="col-md-6">
                 <textarea name="amc_desc" id="amc_desc"  class=" form-control addressfield"  onContextMenu="return false" style="resize:vertical" ><?php echo $sel_result['amc_desc'];?></textarea>
              </div>
            </div>
         ...
         <?php /*?> <div class="form-group">
            <div class="col-md-6"><label class="col-md-6 control-label">Company Model</label>
             <div class="col-md-6">
             <input type="text" name="ven_code" class=" form-control " id="ven_code" value="<?=$sel_result['ven_code']?>"/>
            </div>
            </div>
                <div class="col-md-6"><label class="col-md-6 control-label">Dealar Warranty</label>
             <div class="col-md-6">
            <input type="text" name="dwp" class="digits form-control" id="dwp" value="<?=$sel_result['dwp']?>"/>
            </div>
            </div>
          </div><?php */?>
          <div class="form-group">
            <div class="col-md-12" align="center">
              ...
              <input type="submit" class="btn<?=$btncolor?>" name="add" id="add" value="ADD" title="Add New Model">
              <?php }else{?>
              <input type="submit" class="btn<?=$btncolor?>" name="upd" id="upd" value="Update" title="Update Model Details">
              <?php }?>
              <input type="hidden" name="refid"  id="refid" value="<?=base64_encode($sel_result['model_id'])?>" />
