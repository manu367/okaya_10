...
$get_accbrand = getAccessBrand($_SESSION['userid'],$link1);
$get_accproduct = getAccessProduct($_SESSION['userid'],$link1);
// echo $get_accbrand."get_accbrand <br>";
// echo $get_accproduct;
//die;
?>
<!DOCTYPE html>
...
$(document).ready(function() {
	var dataTable = $('#joblist-grid').DataTable( {
		...
		//"order": [[ 0, "asc" ]],
		"ajax":{
			 url :"../pagination/Admin_eng-stock-grid-data.php", // json datasource
...
<div class="container-fluid">
  <div class="row content">
    ...
    <div class="<?=$screenwidth?> tab-pane fade in active" id="home">
      <h2 align="center"><i class="fa fa-cubes"></i> Engineer Stock </h2>
         ...
         <div class="col-md-6"><label class="col-md-5 control-label">Product</label>	  
			<div class="col-md-6" align="left">
			 <select name="product_name" id="product_name" class="form-control" onChange="document.form1.submit();">
                ...
                <?php
                // echo "SELECT * FROM product_master where status = '1' and product_id in (".$get_accproduct.") order by product_name";
                // die;
        ...
        <div class="form-group">
         ...
         <div class="col-md-6"><label class="col-md-5 control-label">Location</label>	 
			<div class="col-md-6">
                ...
                <option value="">ALL</option>
                <?php
                $res_pro = mysqli_query($link1,"select  distinct(location_code) from locationuser_master  order by location_code desc "); 
                while($row_pro = mysqli_fetch_assoc($res_pro)){
                    $location_val = mysqli_fetch_array(mysqli_query($link1 , "select locationname from location_master where (location_code = '".$row_pro['location_code']."'  or old_location_code = '".$row_pro['location_code']."' )"));
?>
                <option value="<?=$row_pro['location_code']?>" <?php if($_REQUEST['location_code'] == $row_pro['location_code']) { echo 'selected'; }?>><?=$location_val ['locationname']." (".$row_pro['location_code'].")"?></option>
                <?php } ?>
                 </select>
              </div>
              <?php
            //   echo "select userloginid,locusername from locationuser_master";
              //echo "select locusername,location_code,userloginid from locationuser_master  ";
              //echo "select userloginid,locusername from locationuser_master where location_code='RVSASP0930'";
              //"select userloginid,locusername from locationuser_master where location_code='".$_SESSION['userid']."'"
              ?>
          </div>
