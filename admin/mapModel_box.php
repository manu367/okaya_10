  ...
  <div class="row content">
    ...
    <td ><div align="right"><strong>
	  ...
	  <input name="model_name" type="hidden" id="model_name" value="<?=$_REQUEST['model_id']?>">
	     <?php  $rs2=mysqli_query($link1,"select model_id from partcode_master where partcode='".$_REQUEST['partcode']."' ");
			$row2=mysqli_fetch_array($rs2);
			echo getAnyDetails($row2['model_id'],"model","model_id","model_master",$link1);
			echo " (".$_REQUEST['partcode'].")"; ?> </strong></div></td>&nbsp;&nbsp;
    <td ><?php 
	$circlequery="Select distinct(partcode),model_id,part_desc,status from partcode_master where model_id Like '%".$row2['model_id']."%' and (part_category='UNIT'  or part_category='ACCESSORY') and status='1' ";
