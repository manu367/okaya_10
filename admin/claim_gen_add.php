...
//echo "select claim_mnth , location_code from claim_invoice where claim_mnth LIKE '%".$_POST['claim_month']."%' and location_code = '".$locationname."'  and status != '5' ";
  $numcount	= mysqli_query($link1, "select claim_mnth , location_code from claim_invoice where claim_mnth LIKE '%".$_POST['claim_month']."%' and location_code = '".$locationname."' and po_type ='CLAIM'  and status != '5' ");
 ...
 if (mysqli_num_rows($numcount) == 0) {
		...
		$invoice_no = $row_invcount['inv_series']."".$row_invcount['fy']."".str_pad($next_invno,4,"0",STR_PAD_LEFT);
			...
			$map_loc=getAnyDetails($_REQUEST['location_code'],"wh_location","location_code","map_wh_location",$link1);
	...
	for ($i=1;$i<$_POST['row_count'];$i++){
		 ...
		 $sql_claim = "INSERT INTO claim_invoice set location_code='".$_REQUEST['location_code']."',claim_no ='".$invoice_no."',level ='".$_POST[$rep_lvl]."',tot_lvl='".$_POST[$count_lvl]."',price='".$_POST[$level_price]."',value='".$_POST[$cost_avl]."',cgst_per='".$cgst_per."',cgst_amt='".$cgst_val."',sgst_per='".$sgst_per."',sgst_amt='".$sgst_val."',igst_per='".$igst_per."',igst_amt='".$igst_val."',total_cost ='".$tot_val."',claim_date ='".$today."',claim_mnth='".$_POST['claim_month']."',brand='".$_POST[$brand]."' ,area_type='".$_POST[$area_type]."', status = '4',product_id='".$_POST[$product]."',model='".$_POST[$model]."' ";
...
logged_by='".$_SESSION['userid']."',billing_rmk='Against Monthly Claim',bill_from='".$fromlocdet[0]."',from_stateid='".$fromlocdet['5']."',to_stateid='".$tolocdet[5]."' ,bill_to='".$tolocdet[0]."',from_addrs='".$fromlocdet[1]."',disp_addrs='".$fromlocdet[2]."',to_addrs='".$tolocdet[1]."',deliv_addrs='".$tolocdet[3]."',status='4',document_type='INV',basic_cost='".$basic_cost."',  total_cost='".$inv_tot_cost."',cgst_amt='".$cgst_final_val."',sgst_amt='".$sgst_final_val."',igst_amt='".$igst_final_val."',po_type='CLAIM' ,claim_month='".$_POST['claim_month']."' "; 
	...
	//echo "UPDATE job_claim_appr set claim_no='".$invoice_no."'  where action_by = '".$_REQUEST['location_code']."'  and  app_status='Y' and hand_date like '%".$_POST['claim_month']."%'";
		$res_job_claim = mysqli_query($link1,"UPDATE job_claim_appr set lb_claim_no='".$invoice_no."'  where action_by = '".$_REQUEST['location_code']."'  and  app_status='Y' and hand_date like '%".$_POST['claim_month']."%'");
	if(!$res_job_claim){
		$flag = false;
...
//	echo "INSERT INTO location_account_ledger set location_code='".$_REQUEST['location_code']."',entry_date='".$today."',remark='".$invoice_no."', transaction_type = 'Monthly Claim',month_year='".date("m-Y")."',crdr='CR',amount='".$inv_tot_cost."'";
	$res_ac_ledger = mysqli_query($link1,"INSERT INTO location_account_ledger_claim set location_code='".$_REQUEST['location_code']."',entry_date='".$today."',remark='".$invoice_no."', transaction_type = '".$narreation."',month_year='".$_POST['claim_month_eg']."',crdr='CR',amount='".$inv_tot_cost."'");
	if(!$res_ac_ledger){
		$flag = false;
...
if ($flag) {
        mysqli_commit($link1);
        $msg = "Claim has been  generated ".$invoice_no;
		$cflag = "success";
		$cmsg = "Success";
    } 
