...
$res = mysqli_fetch_array(mysqli_query($link1,"select reference from sf_lead_master where lid = '".$_REQUEST['id']."' "));
...
if($_POST['Submit']=='Save'){
	...
	if($party_id != "" && $party_add != "" && $locationstate != "" && $locationcity != "" && $circle != ""){
		...
		$qry2 = "INSERT INTO sf_quote_master SET quote_no = '".$auto_quote_no."', party_id = '".$party_id."', sales_executive = '".$designation."', address = '".$party_add."', state = '".$locationstate."',  city = '".$locationcity."', country = '".$circle."', ref_no = '".$reference_no."', create_dt = '".$today."', remark = '".$remark."', qty = '".$total_qty."', total_amt = '".$sub_total."', total_taxamt = '".$total_tax."',  grandtotal = '".$grand_total."', create_time = '".$currtime."', create_by =  '".$_SESSION['userid']."',  ip = '".$ip."', status = '7' , lead_ref_no = '".$lead_no."'  ";
	...
	$qry3 = "insert into sf_status_history set party_id='".$party_id."', status_id='7', trans_type='add_quote', trans_no='".$auto_quote_no."'";	
	...
	if ($flag) {
        mysqli_commit($link1);
        $msg = "Quote is successfully added with ref. no. ".$auto_quote_no;
    } else {
              ...
              <tr id='addr0'>
                <td class="col-md-3">
                  ...
                  <select name="prod_code[0]" id="prod_code[0]" class="form-control selectpicker required" required data-live-search="true"  onChange="checkDuplicate(0, this.value);"   >
                    ...
                    <?php 
				$model_query="select productcode,productname,productcolor from product_master where status='1'";
