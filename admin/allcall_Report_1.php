...
<div class="container-fluid">
  <div class="row content">
        ...
        <div class="col-md-6"><label class="col-md-5 control-label">Date Type</label>	  
			<div class="col-md-6">
                   <select name="date_type" id="date_type" class="form-control">
					<option value="open_date" <?php if($_REQUEST['date_type']=='open_date') { echo "selected"; }?>>Open Date</option>
                    <option value="close_date" <?php if($_REQUEST['date_type']=='close_date') { echo "selected"; }?>>Close Date</option>
	</select>
              </div>
              </div>
		  <div class="col-md-6"><label class="col-md-5 control-label">Status</label>	  
			<div class="col-md-6" id="modeldiv">
				...
				<?php		
                $res_status = mysqli_query($link1,"select  status_id , main_status_id,display_status from jobstatus_master where status_id in ('1','2','3','7','8','10','12','48','81','82') and (status_id = main_status_id )")or die(mysqli_error($link1)); 
                while($row_status = mysqli_fetch_assoc($res_status)){?>
                <option value="<?=$row_status['status_id']?>" <?php for($i=0; $i<count($statusarray); $i++){ if($statusarray[$i]==$row_status['status_id']){ echo "selected";}}?>><?=$row_status['display_status']?></option>
                <?php } ?>
			</select>
              </div>
          </div>
	...
	if ($_REQUEST['statename'] == '') {?>		
			<?php  }else {?>
               Download Excel : <a href="../excelReports/allcallexcel_csv.php?daterange=<?=$_REQUEST['daterange']?>&location_code=<?=base64_encode($_REQUEST['locationname']);?>&status=<?=base64_encode($statusstr);?>&pending=<?=base64_encode($_REQUEST['pending'])?>&state=<?=base64_encode($statestr);?>&proid=<?=base64_encode($prostr);?>&date_type=<?=$_REQUEST['date_type']?>" title="Export All Call details in excel"><i class="fa fa-file-excel-o fa-2x faicon" title="Export All Call details in excel"></i>All Call Report</a>
			 <?php /*?> <a href="../excelReports/allcallexcel_new1.php?daterange=<?=$_REQUEST['daterange']?>&location_code=<?=base64_encode($_REQUEST['locationname']);?>&modelid=<?=base64_encode($modelstr);?>&status=<?=base64_encode($statusstr);?>&pending=<?=base64_encode($_REQUEST['pending'])?>&state=<?=base64_encode($statestr);?>&proid=<?=base64_encode($prostr);?>&brand=<?=base64_encode($brandstr);?>" title="Export All Call details in excel"><i class="fa fa-file-excel-o fa-2x faicon" title="Export All Call details in excel"></i></a><?php */?>
			   <?php }?>
          </div>
	    </div><!--close form group-->
			...
			<table class="table table-bordered" width="100%">
				<tbody>          
					...
					<?php
						...
						$uj = mysqli_fetch_array(mysqli_query($link1,"Select count(job_id) as jn  from jobsheet_data where current_location = '' and (open_date >= '".$fromdate."' and open_date <='".$todate."') ".$st." ".$stateid." ".$product_id." "));
					?>
					<!---- Unallocated Jobs ---->
					<!----
					...
					<?php	
						...
						$qrr = mysqli_query($link1,"Select current_location  from jobsheet_data where ".$locationcode." and (open_date >= '".$fromdate."' and open_date <='".$todate."') ".$st." ".$stateid." ".$product_id." group by current_location order by current_location ");
						...
						///// asp ////
						$c1 = mysqli_fetch_array(mysqli_query($link1,"Select count(job_id) as jn  from jobsheet_data where current_location = '".$rw['current_location']."' and status in ('1','2','5','7','50','56','58') and (open_date >= '".$fromdate."' and open_date <='".$todate."') ".$stateid." ".$product_id." "));
						$pending = $c1['jn'];
						///// pna ////
						$c2 = mysqli_fetch_array(mysqli_query($link1,"Select count(job_id) as jn  from jobsheet_data where current_location = '".$rw['current_location']."' and status in ('3') and (open_date >= '".$fromdate."' and open_date <='".$todate."') ".$stateid." ".$product_id." "));
						$pna = $c2['jn'];
						///// repair ////
						...
						$demo = $c4['jn'];*/
						///// installation ////
						$c5 = mysqli_fetch_array(mysqli_query($link1,"Select count(job_id) as jn  from jobsheet_data where current_location = '".$rw['current_location']."' and status in ('49') and (open_date >= '".$fromdate."' and open_date <='".$todate."') ".$stateid." ".$product_id." "));
						$installation = $c5['jn'];
						///// cancel ////
						$c6 = mysqli_fetch_array(mysqli_query($link1,"Select count(job_id) as jn  from jobsheet_data where current_location = '".$rw['current_location']."' and status in ('12') and (open_date >= '".$fromdate."' and open_date <='".$todate."') ".$stateid." ".$product_id." "));
						$cancel = $c6['jn'];
						///// rwr ////
						$c7 = mysqli_fetch_array(mysqli_query($link1,"Select count(job_id) as jn  from jobsheet_data where current_location = '".$rw['current_location']."' and status in ('11') and (open_date >= '".$fromdate."' and open_date <='".$todate."') ".$stateid." ".$product_id." "));
						$rwr = $c7['jn'];
						///// total ////
						
						$c8 = mysqli_fetch_array(mysqli_query($link1,"Select count(job_id) as jn  from jobsheet_data where current_location = '".$rw['current_location']."' and (open_date >= '".$fromdate."' and open_date <='".$todate."') ".$stateid." ".$product_id." "));
						$sum_of = $c8['jn'];

						$total = $sum_of;
						
						$oth_job = ($total - ( $pending + $pna  + $installation + $cancel + $rwr));
