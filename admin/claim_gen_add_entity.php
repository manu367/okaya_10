...
//echo "select claim_mnth , location_code from claim_invoice where claim_mnth LIKE '%".$_POST['claim_month']."%' and location_code = '".$locationname."'  and status != '5' ";
  $numcount	= mysqli_query($link1, "select claim_mnth , location_code from claim_invoice where claim_mnth LIKE '%".$_POST['claim_month']."%' and location_code = '".$locationname."' and po_type ='ENTITY LABOUR CLAIM'  and status != '5' ");
 ...
 if (mysqli_num_rows($numcount) == 0) {
	...
	for ($i=1;$i<$_POST['row_count'];$i++){
		 ...
		 $area_type="area_type".$i;
		 $entity_type="entity".$i;
		 $rep_lvl="rep_lvl".$i;
		 $count_lvl="count_lvl".$i;
		 $level_price="level_price".$i;
		 $cost_avl="cost_avl".$i;
		 	$res_tax = mysqli_query($link1,"SELECT id,sgst,igst,cgst FROM tax_hsn_master where hsn_code='998716'");
		 ...
		 $sql_claim = "INSERT INTO claim_invoice set location_code='".$_REQUEST['location_code']."',claim_no ='".$invoice_no."',level ='".$_POST[$rep_lvl]."',tot_lvl='".$_POST[$count_lvl]."',price='".$_POST[$level_price]."',value='".$_POST[$cost_avl]."',cgst_per='".$cgst_per."',cgst_amt='".$cgst_val."',sgst_per='".$sgst_per."',sgst_amt='".$sgst_val."',igst_per='".$igst_per."',igst_amt='".$igst_val."',total_cost ='".$tot_val."',claim_date ='".$today."',claim_mnth='".$_POST['claim_month']."',entity_type='".$_POST[$entity_type]."' ,area_type='".$_POST[$area_type]."', status = '4' ";
	...
	echo $sql_billmaster = "INSERT INTO billing_master set from_location='".$_REQUEST['location_code']."', to_location='".$map_loc."',from_gst_no='".$fromlocdet[8]."',to_gst_no='".$tolocdet[8]."',
party_name='".$tolocdet[0]."',challan_no='".$invoice_no."',sale_date='".$today."',entry_date='".$today."',entry_time='".$currtime."',
logged_by='".$_SESSION['userid']."',billing_rmk='Against Monthly Claim',bill_from='".$fromlocdet[0]."',from_stateid='".$fromlocdet['5']."',to_stateid='".$tolocdet[5]."' ,bill_to='".$tolocdet[0]."',from_addrs='".$fromlocdet[1]."',disp_addrs='".$fromlocdet[2]."',to_addrs='".$tolocdet[1]."',deliv_addrs='".$tolocdet[3]."',status='4',document_type='INV',basic_cost='".$basic_cost."',  total_cost='".$inv_tot_cost."',cgst_amt='".$cgst_final_val."',sgst_amt='".$sgst_final_val."',igst_amt='".$igst_final_val."',po_type='ENTITY LABOUR CLAIM' ,claim_month='".$_POST['claim_month']."' "; 
	...
	if (!$res_billmaster) {
	 ...
	 $narreation= "Monthly Claim" ."-".$_POST['claim_month_eg'];
	...
	//echo "UPDATE job_claim_appr set claim_no='".$invoice_no."'  where action_by = '".$_REQUEST['location_code']."'  and  app_status='Y' and hand_date like '%".$_POST['claim_month']."%'";
		$res_job_claim = mysqli_query($link1,"UPDATE job_claim_appr set enty_claim_no='".$invoice_no."'  where action_by = '".$_REQUEST['location_code']."'  and  app_status='Y' and hand_date like '%".$_POST['claim_month']."%'");
	if(!$res_job_claim){
		$flag = false;
...
//	echo "INSERT INTO location_account_ledger set location_code='".$_REQUEST['location_code']."',entry_date='".$today."',remark='".$invoice_no."', transaction_type = 'Monthly Claim',month_year='".date("m-Y")."',crdr='CR',amount='".$inv_tot_cost."'";
	$res_ac_ledger = mysqli_query($link1,"INSERT INTO location_account_ledger set location_code='".$_REQUEST['location_code']."',entry_date='".$today."',remark='".$invoice_no."', transaction_type = '".$narreation."',month_year='".$_POST['claim_month_eg']."',crdr='CR',amount='".$inv_tot_cost."'");
	if(!$res_ac_ledger){
		$flag = false;
