...
if(!empty($_POST['newpwd'])){
	/////// check old password is correct or not
	 $chkquery="Select  location_code from location_master where location_code='".$_SESSION['asc_code']."' and pwd='".$_POST['oldpwd']."'";	
	$check=mysqli_query($link1,$chkquery);
	//// old password should be matched
	if(mysqli_num_rows($check)==1){
	  ///// check password should not be equal to userid
	  if($_POST['newpwd']!=$_SESSION['asc_code']){
		///// if everything is OK then proceed to change password
	 $query="UPDATE location_master set pwd='".$_POST['newpwd']."' where location_code='".$_SESSION['asc_code']."' and pwd='".$_POST['oldpwd']."'";
