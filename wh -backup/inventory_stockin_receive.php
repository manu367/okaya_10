...
##################
/// BY HEMANT, 2024
$po_sql = "select * from billing_master where challan_no='".$docid."' and po_type in ('PNA','PO','Opening Stock','P2C') AND status='3'";
				...
				if(mysqli_num_rows($res_poData) > 0)
				{
					while($row_poData=mysqli_fetch_assoc($res_poData))
					{
						...
						if($flag)
						{
							if($ok_qty > 0)
							{
								$flag_h1 = stockLedgerO($ref_no, $today, $row_poData['partcode'], $po_row['to_location'], $po_row['from_location'], "IN", "OK", "Stock In", "Receive", $ok_qty, $row_poData['price'], $_SESSION['userid'], $today, $currtime, $ip, $link1, $flag,$po_row['to_location']);
								if(!$flag_h1)
								{
									$flag = false;
						...
						if($flag)
						{
							if($dam_qty > 0)
							{
								$flag_h2 = stockLedgerO($ref_no, $today, $row_poData['partcode'], $po_row['to_location'], $po_row['from_location'], "IN", "DAMAGE", "Stock In", "Receive", $dam_qty, $row_poData['price'], $_SESSION['userid'], $today, $currtime, $ip, $link1, $flag,$po_row['to_location']);
								if(!$flag_h2)
								{
									$flag = false;
