...
$count_serial=mysqli_num_rows(mysqli_query($link1,"select id from imei_details where challan_no='".$po_no."' and partcode='".$partcode."'"));
...
if ($_POST['button'] == 'SUBMIT') {
    ...
    $res = mysqli_query($link1,"select id,status from imei_details where imei1='".$_POST['serialno']."' and location_code= '".$_SESSION['asc_code']."' and status ='1'  order by id desc");
...
if ($_POST['submit'] == 'Save') {
	...
	///// check serial no. in stock
    $res = mysqli_query($link1,"select id,partcode,status from imei_details where imei1='".$_POST['change_bar']."' and location_code= '".$_SESSION['asc_code']."' and status ='1'  order by id desc");
	$row_checkstock=mysqli_fetch_assoc($res);
	$disp_box_qty=1;
	///// check serial no. with in the temp table
...
if($_POST['final_sub']=='PROCESS'){
	...
	while($row_serial=mysqli_fetch_array($get_serial)){
		...
		mysqli_query($link1,"update imei_details  set status='3',dis_date='".$today."',challan_no='".$po_no."' where  imei1='".$row_serial['serial_no']."'  or imei2='".$row_serial['serial_no']."'");
	
		///// update serial no. inventroy
       mysqli_query($link1,"INSERT INTO imei_history set imei1='".$row_serial['serial_no']."',partcode='".$row_serial['partcode']."',transaction_no='".$po_no."',remark='IMEI Dispatched' ,location_code='".$_SESSION['asc_code']."' ");    
		mysqli_query($link1,"delete from temp_barcode_tnx where id='".$row_serial['id']."'");
	}
	/////// update attachment flag
	$checkboxqty=mysqli_fetch_assoc(mysqli_query($link1,"select COUNT(id) as dispqty from imei_details where challan_no='".$po_no."' and partcode='".$partcode."'"));
	if($checkboxqty['dispqty']==$qty){
       mysqli_query($link1,"update billing_product_items set attach_file='BY SCANNING',imei_attach='Y' where challan_no='".$po_no."' and partcode='".$partcode."'");

	$cflag = "success";
	$cmsg = "Success";
	$msg = "IMEI is successfully uploaded against.".$partcode;
	}
	header("Location:invoice_challan_view.php?refid=".base64_encode($po_no)."&msg=".$msg."&chkflag=".$cflag."&chkmsg=".$cmsg."&daterange=".$today."&status=".$_REQUEST['grn_type']."&doc_type=".$_REQUEST['grn_type']."".$pagenav);
	exit;
}
?>
<!DOCTYPE html>
...
<div class="container-fluid">
 <div class="row content">
   ...
   <div class="<?=$screenwidth?> tab-pane fade in active">
      <h2 align="center"><i class="fa fa-shower"></i>  Scan IMEI For PO</h2>
